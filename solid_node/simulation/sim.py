...
from .driver import driver_states
...
class Sim:
    ...
    Construction binds every declared driver's default through
    `set_state`, so an assembly whose render() reads a driver is
    rendered under a complete snapshot from the very first render
    rather than failing on an unbound entry. That is the resolution
    stage 1 deferred to this layer: the framework invents no defaults,
    the declarations state them.
    ...
    def __init__(self, node, dt, meshes=False):
        ...
        self.dt = dt
        self.tick = 0
        self.drivers = driver_states(type(node))
        self.trajectory = []
        self._at = {}
        self._every = []
    ...
    @property
    def state(self):
        """The current snapshot: a fresh dict, so a caller holding one
        holds a value and not a view of the running simulation."""
    ...
    def _instruction(self, name):
        instructions = getattr(self.node, 'instructions', {})
        try:
            return instructions[name]
        except KeyError:
            known = ', '.join(sorted(instructions)) or 'none'
            raise KeyError(f"no instruction '{name}' declared on "
    ...
    def _driver(self, name, instruction_name):
        try:
            ...
            raise KeyError(f"instruction '{instruction_name}' targets driver "
                           f"'{name}', which {type(self.node).__name__} does "
                           f'not declare; declared: {known}') from None
