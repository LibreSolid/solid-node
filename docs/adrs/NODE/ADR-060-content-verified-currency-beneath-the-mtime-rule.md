...
**Amended by:** [ADR-071: Node-Scoped Content Currency](./ADR-071-node-scoped-content-currency.md) — the digest unit is the file as one node sees it, not the file; [ADR-081: Per-Contributor Metadata Guards Aggregate-Mtime Currency](./ADR-081-per-contributor-metadata-guards-aggregate-mtime-currency.md) — the digest fallback also runs when a recorded source-set fingerprint differs or is absent
**Depends on:**
