...
- [ ] 1.1 Bring the bench environment to the framework's pins (cadquery
...
- [ ] 2.1 Write `tests/test_sheet_leaf.py` covering the `sheet-parts`
...
- [ ] 2.2 Extend adapter-contract coverage red: `Build123dSheetNode.exact`
...
- [ ] 3.1 Add `SheetLeafNode` (abstract, under `ExactLeafNode`): owns
...
- [ ] 3.2 Add `Build123dSheetNode`: `namespace = 'build123d'`; accepts
  `Sketch`, `Face` or `BuildSketch` builder from `profile()`, normalised
  to one `Face`; extrusion in build123d terms
- [ ] 3.3 Add the DXF artifact: `dxf_file` path beside `stl_file`;
...
- [ ] 3.4 Export the new classes from `solid_node.node` without importing
  build123d eagerly beyond what the existing adapters already cost

## 4. Green and validation

- [ ] 4.1 Run the new tests green and the full framework suite; confirm
  the pre-existing pass/fail set is otherwise unchanged
- [ ] 4.2 Build a representative caller: a minimal panel-with-holes sheet
...
- [ ] 5.1 Update `docs/leaf-nodes.rst` with the sheet leaf kind: profile
  contract, thickness, the nominal DXF artifact, and what is deliberately
  not yet covered (kerf, SVG, engraving)
- [ ] 5.2 After implementation confirms the design, extract the ADR for
