...
- [ ] 1.1 **Fixtures.** A `TwoFreedom` node class declaring, in this
  ...
  orders.
- [ ] 1.2 **Two joints, either binding order.** Both benches produce the
  ...
  is wrong.
- [ ] 1.3 **Relation-bound joints.** The cycloidal case: `spin` then
...
- [ ] 1.4 **Re-binding one joint of several.** Bind `pivot`, bind
...
- [ ] 1.5 **Inheritance and the enumerator.** A base declaring `a` then
...
- [ ] 1.6 **Sweep across runs, with the application order reversed
  ...
  base.
- [ ] 1.7 **Two independent animators on one node.** Assembly A binds the
...
- [ ] 1.8 **Contiguity, and the guard for the stacked cycles.** A
...
- [ ] 1.9 **Hand-written motion is outside the joint block.** A bench
...
- [ ] 1.10 **The two existing assertions invert.** MODIFY, do not delete:
  ...
  - `MotionDisciplineTest::test_hand_written_and_joint_motion_coexist_in_order`
    ...
    reads as a revert.
- [ ] 1.11 **Serialization.** `solid_node.core.serializer.serialize` on a
...
- [ ] 2.1 `solid_node/node/base.py`: replace `apply_motion(node,
...
- [ ] 2.2 `solid_node/node/base.py`: update the motion-block comment
...
- [ ] 2.3 `solid_node/motion/joints.py`: `Joint.place` computes its slot
...
- [ ] 2.4 `solid_node/motion/joints.py`: state the ordering contract in
...
- [ ] 2.5 Run task 1. Everything green. Then the whole suite:
...
- [ ] 3.1 `docs/adrs/NODE/ADR-093-<slug>.md`: "The joints of one class
...
- [ ] 3.2 `docs/adrs/README.md`: the ADR-093 row in the NODE section in
  chronological order, and "extended by 093" on ADR-088's row.
- [ ] 3.3 `docs/architecture.md` §Joints (lines 484-512): rewrite the
...
- [ ] 3.4 `docs/changelog.rst`, `Unreleased` section: one entry at the
...
- [ ] 3.5 `docs/driving.rst`, the paragraph ending "both forms may sit on
  ...
  still true.
- [ ] 3.6 Sync the delta into `openspec/specs/joints/spec.md`.

## 4. The plan note and the findings

- [ ] 4.1 `workflow/docs/composed-joints.md`: mark §4 taken up by this
  cycle, and note that the OpenSpec change is now the authority for it.
  Leave §5 and §6 provisional.
- [ ] 4.2 `workflow/warts.md`: mark the seven composition-order sightings
...
- [ ] 4.3 `libresolid-studio/docs/motion-general-refactor.md` is SHOP
  material in a different repository. Do not edit it from this cycle;
  report what it needs and let the pilot place it.
...
- [ ] 5.1 **Which framework tree each capture runs against.** The
...
- [ ] 5.1a For each of the sixteen migrated projects named in
...
- [ ] 5.2 **Acceptance: maximum deviation 0** on every project and every
  ...
  printed.
- [ ] 5.3 Any non-zero deviation STOPS the cycle: report which node, which
...
- [ ] 5.4 Note honestly in the evidence what this does NOT cover: the
  ...
  comparison replaces.
- [ ] 5.5 Pixels: `solid snapshot` on two models that carry more than one
...
- [ ] 6.1 **The same-slot survivor.** Write the test that restores a node
...
- [ ] 6.2 **A legacy render inside a joint's frame.** Determine whether
...
- [ ] 6.3 Confirm before archiving that neither open question changed the
