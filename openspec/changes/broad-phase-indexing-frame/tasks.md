...
- [ ] 1.1 Test helpers: `assembly_records(placements)` building real
...
- [ ] 1.2 `IndexingFrameCompletenessTest`: run `BoundaryTableCompletenessTest`'s
...
- [ ] 1.3 A bound taken in a non-world frame encloses the placed geometry
...
- [ ] 1.4 **The clock-shaped scenario.** An assembly of several boxes whose
...
- [ ] 1.5 **Flush contact survives the frame change.** Take the
...
- [ ] 1.6 Determinism: the same records indexed twice choose the same frame
...
- [ ] 1.7 The candidate set, the strict preference, and the tie rule: with
...
- [ ] 1.8 Guards: a candidate solid whose placement matrix is singular is
...
- [ ] 1.9 An axis-aligned assembly is indexed EXACTLY as today: assert the
...
- [ ] 1.10 The support path still reads world boxes. Assert directly that a
...
- [ ] 1.11 Every existing test in `tests/test_broad_phase_culling.py` and
...
- [ ] 2.1 `solid_node/test.py`: `_INDEXING_FRAME_CANDIDATES = 3` and
  ...
  padded.
- [ ] 2.2 `_framed_bounds(local_bounds, matrix, inverse_frame)` beside
...
- [ ] 2.3 `_indexing_frame_boxes(records)` (or equivalently named): build the
  ...
  carry it.
- [ ] 2.4 `_place_solid`: append `local_bounds` as the eighth field
...
- [ ] 2.5 `assertNoSolidInterference`: index on `_indexing_frame_boxes(solids)`
...
- [ ] 2.6 Confirm by reading — and note in the commit message — that
...
- [ ] 3.1 `tests/test_broad_phase_culling.py` green.
- [ ] 3.2 `tests/test_adaptive_broad_phase.py` green, unedited.
- [ ] 3.3 `tests/test_assembly_supported.py` and `tests/test_sparse_statics.py`
  green.
- [ ] 3.4 `tests/test_intersection_memo.py`, `tests/test_exact_geometry.py`,
  `tests/test_persistent_piece_facts.py` (it calls
  `_placed_assembly_solids` directly) and `tests/test_meta.py` green.
- [ ] 3.5 The whole suite green, and the three statics tests named in 1.10 run
  explicitly with their result recorded in the commit message.

## 4. Documentation and the decision record

- [ ] 4.1 `docs/adrs/TEST-FRAMEWORK/ADR-091-the-broad-phase-chooses-its-indexing-frame.md`,
...
- [ ] 4.3 `docs/architecture.md`: rewrite the broad-phase sentences of the
  ...
  otherwise alone.
- [ ] 4.4 `docs/testing.rst`: the two "conservative world AABB" sentences,
...
- [ ] 4.6 `workflow/warts.md`, the wall-clock-02 section's second bullet:
...
- [ ] 5.1 **Candidate-pair count at one instant, cheap.** From
  ...
  evidence.
- [ ] 5.2 **The chooser's own numbers, at the same instant.** In the same
...
- [ ] 5.3 **If the fused plates are NOT among the candidates**, record that in
...
- [ ] 5.4 **The swing sweep, end to end, after.** Run
...
- [ ] 5.6 Write `openspec/changes/broad-phase-indexing-frame/evidence.md`: the
...
- [ ] 6.1 `openspec validate broad-phase-indexing-frame --strict` passes.
- [ ] 6.2 Sync the modified `test-framework` baseline spec and archive the
