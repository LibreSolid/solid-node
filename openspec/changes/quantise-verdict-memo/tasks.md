...
- [ ] 1.1 In `tests/test_intersection_memo.py`, extend `MemoTestCase` so a
...
- [ ] 1.2 A pair carried together through a parent is served from the memo:
...
- [ ] 1.3 A pair displaced by more than the quantum is recomputed: same pair,
  one node translated by 10× the quantum, assert the boolean runs.
- [ ] 1.4 `--placement-quantum 0` restores the exact key: the 1.2 scenario
  under a policy with quantum `0` recomputes.
- [ ] 1.5 Signed zero does not split a cell: two relative matrices whose
...
- [ ] 1.6 The exact kernel quantises too: the 1.2 scenario with two exact
  nodes under an exact policy, asserting no OCCT boolean runs. Reuse the
  exact fixtures of `tests/test_exact_geometry.py`.
- [ ] 1.7 A non-finite relative matrix yields no key: `_verdict_key` with a
  singular matrix returns `None` and the comparison is computed.
- [ ] 1.8 Two quanta in one process never cross-serve: build a key at
...
- [ ] 1.9 The placed geometry is not quantised. On a memo HIT nothing is
...
- [ ] 1.10 Keep every existing scenario in the file green as written,
  flush contact included, now under the default quantum.

## 2. Red first: the option

- [ ] 2.1 In `tests/test_manager_test.py`, `ComparisonKernelSelectionTest`:
...
- [ ] 2.2 The seven POSITIONAL two-argument constructions of
  ...
  (`TypeError`).
- [ ] 2.3 `--placement-quantum` parses into `args.placement_quantum`, and is
  `None` when absent; it is NOT part of the `--exact`/`--faceted` mutually
  exclusive group.
- [ ] 2.4 Resolution: the flag beats `SOLID_TEST_PLACEMENT_QUANTUM` beats the
  ...
  kernels.
- [ ] 2.5 Errors: a negative flag value and a negative environment value each
  ...
  mm.
- [ ] 2.6 The summary line: unchanged at the default quantum (assert the
...
- [ ] 3.1 `solid_node/test.py`: add `DEFAULT_PLACEMENT_QUANTUM = 1e-9` and
...
- [ ] 3.2 `resolve_comparison_policy`: resolve and validate the quantum
...
- [ ] 3.3 `_verdict_key`: refuse a non-finite relative matrix with `None`;
...
- [ ] 3.4 Confirm by reading — and note in the commit message — that
...
- [ ] 3.5 `solid_node/manager/test.py`: add `--placement-quantum MM`
...
- [ ] 3.6 Run `tests/test_intersection_memo.py`, `tests/test_manager_test.py`,
...
- [ ] 4.1 `docs/adrs/TEST-FRAMEWORK/ADR-090-the-placement-quantum-is-a-property-of-the-test-run.md`,
...
- [ ] 4.2 `docs/adrs/README.md`: add the row in the TEST-FRAMEWORK section in
  chronological order — `**Accepted**, amends 070` — and add "amended by
  090" to ADR-070's row.
- [ ] 4.3 `docs/architecture.md`: rewrite the memo paragraph (the one reading
...
- [ ] 4.4 `docs/testing.rst`: a subsection beside the comparison-kernel one
  covering the quantum, per the user-documentation delta.
- [ ] 4.5 `docs/cli.rst`: `--placement-quantum MM` in the `solid test`
  synopsis and option list, and `SOLID_TEST_PLACEMENT_QUANTUM` in the
  environment section.
- [ ] 4.6 `docs/changelog.rst` "Unreleased".
- [ ] 4.7 `workflow/warts.md`: mark the first wall-clock-02 finding as fixed
  by this cycle, with the measured before/after, leaving the other findings
  as they stand.

## 5. Validation in the originating project

- [ ] 5.1 From `/home/asa/devel/libresolid-studio/projects/3DPrintedClocks`,
...
- [ ] 5.2 Repeat 5.1 with `--placement-quantum 0` appended to `sys.argv`,
...
- [ ] 5.3 Write `openspec/changes/quantise-verdict-memo/evidence.md`: the
...
- [ ] 5.4 Confirm the test's verdicts are identical before and after — same
  passes, same failures, same messages — and say so in `evidence.md`.
  Nothing is committed in the project repository by this cycle.

## 6. Close the cycle

- [ ] 6.1 `openspec validate quantise-verdict-memo --strict` passes.
- [ ] 6.2 Sync the two modified baseline specs and archive the change, per
