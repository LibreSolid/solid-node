...
- [ ] 0.3 Record the BASE document of the acceptance project: build
...
- [ ] 0.4 Probe and paste the three facts the design is written against:
...
- [ ] 1.1 `tests/test_running_document.py` (new): a running root's export
  manifest and a running root's built `viewer.json` each declare
  `version: 5` and carry a `program` key. RED.
- [ ] 1.2 Same file: an UNTIMED root's and a LOOPING root's documents carry
...
- [ ] 2.4 Same: after serialization the tree is left as it was found —
...
- [ ] 2.5 Same: every free name every operation and `params` expression
...
- [ ] 2.6 `symbolic_document`: under a running root, install a `RunBinder`
...
- [ ] 3.1 `tests/test_running_document.py`: a running `Train`'s
  ...
  nothing else. RED.
- [ ] 3.2 Same: every `kind == "input"` id is a key of the document's own
...
- [ ] 3.3 Same: `program.edges` matches `sim.program.edges` one for one, in
...
- [ ] 3.4 Same, on `Window`: the published plan's `jumps` is the graph's
...
- [ ] 3.5 Same, the collision 0.4(c) found: a running root carrying THREE
...
- [ ] 3.6 Same: `program.spans` on `Ratchet` carries an expression `low`
...
- [ ] 3.7 Same: no `program` expression carries producer-local sharing
  syntax (`let(`), and a subexpression shared between a law and its own
  plan's level appears once in `bindings`. RED.
- [ ] 3.8 Same: a running model published twice is byte-identical, the
  program's ordering and its minted names included. RED.
- [ ] 3.9 `Program.published()` in `program.py`: the projection, with
  ...
  green.
- [ ] 3.10 The ACCEPTANCE case, as a PROBE rather than a suite test — the
...
- [ ] 4.1 `tests/test_running_document.py`: a running root whose
...
- [ ] 5.1 `tests/test_running_corpus.py` (new): load
  ...
  fixture yet.
- [ ] 5.2 Same file: each machine's REAL published document reproduces the
  fixture's `program`, `drivers`, `instructions` and `bindings`. RED.
- [ ] 5.3 Same file: `uncovered_features` is exercised directly — a corpus
  whose machines contain no `%` law is refused naming the uncovered
  feature; one covering everything is accepted. RED.
- [ ] 5.4 `tools/generate_running_corpus.py` (new), modelled on
...
- [ ] 5.5 Record in `evidence.md`: the fixture's byte size, the machine
  count, the tick count per machine, and the generator's runtime.

## 6. Two `Sim`s over one tree — RED FIRST

- [ ] 6.1 `tests/test_running_simulation.py`: two `Sim`s constructed over
...
- [ ] 6.2 Same file: the FIRST simulation, stepped after the second was
  constructed, refuses naming both and saying its bank no longer describes
  the tree. RED.
- [ ] 6.3 Same file: the existing author-binding refusal (`HandBound`) is
...
- [ ] 6.4 `Sim.__init__` releases a previous run's ownership before
...
- [ ] 7.1 `tests/test_viewer_bundle.py`: `bundle.document_versions()`
  returns the report's `documentVersions`, and `[1, 2, 3, 4]` when the
  field is absent. RED.
- [ ] 7.2 `tests/test_manager_viewer.py`: `solid viewer` prints the field
  when the installed viewer reports it. RED.
- [ ] 7.3 `tests/test_browser_renderer.py`: the web renderer on a running
...
- [ ] 7.4 `tests/test_export.py` and `tests/test_build_publication.py`: a
  ...
  RED.
- [ ] 7.4b `tests/test_docs_exports.py`: the Sphinx directive embedding a
...
- [ ] 7.5 Implement: `bundle.document_versions()`; the refusal in
...
- [ ] 8.1 `tests/test_snapshot.py` and `tests/test_cli.py`: `--drive
...
- [ ] 8.2 Same: `--time 0.5` on a running root fails naming `--drive`;
  `--time 0.0` (and the default) is accepted and keyframes zero; `--time`
  under a looping and an undeclared root is unchanged. RED.
- [ ] 8.3 Implement in `manager/snapshot.py`: the `--drive` option on that
...
- [ ] 9.1 `docs/scenarios.rst`: fix the dangling
...
- [ ] 9.2 `docs/viewer.rst` and `docs/embedding.rst`: document version 5,
...
- [ ] 9.3 `docs/cli.rst`: `--drive`, and `--time` on a running root.
- [ ] 9.4 `docs/api-reference.rst`: `Program.published()` and the corpus
  generator; `docs/changelog.rst`; `docs/architecture.md`'s Export and
  Simulation sections.
- [ ] 9.5 `docs/expression-graphs.rst`: the program's expressions share the
  document's bindings table, and placeholders are a published name kind.
- [ ] 9.6 ADR-110 (EXPORT) and ADR-111 (EXPORT) as design §11 states them,
  with `docs/adrs/README.md`'s index updated in chronological order; a
  dated amendment on ADR-105 for the ownership repair.
- [ ] 9.7 `workflow/open-run-simulation/roadmap.md`: the cycle-4 line —
...
- [ ] 10.1 Full suite, exact counts, against 0.2's baseline. Every failure
  explained; no test deleted or weakened to pass.
- [ ] 10.2 The UNTIMED byte-identity check of 1.2 passes against documents
  ...
  root.
- [ ] 10.3 `openspec validate publish-the-mechanical-program --strict`
  passes; `evidence.md` carries every probe, count and measurement this
  file asks for.
- [ ] 10.4 Nothing is written in `projects/`, in `solid-node-viewer/`, or
