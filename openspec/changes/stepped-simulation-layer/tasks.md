...
- [ ] 1.1 Red: new `tests/test_simulation_drivers.py` covering the
      ...
      two fresh runs
- [ ] 1.2 Implement `solid_node/simulation/driver.py`: frozen `Driver`
...
- [ ] 1.3 Green: driver tests pass

## 2. Instructions and the Sim loop

- [ ] 2.1 Red: new `tests/test_simulation_sim.py` covering — defaults
      ...
      records every tick
- [ ] 2.2 Implement `solid_node/simulation/instruction.py` and
      `solid_node/simulation/sim.py` per design decisions 2–5
- [ ] 2.3 Green: sim tests pass; full suite green

## 3. Scenario testing

- [ ] 3.1 Red: minimal axis fixture project under `tests/` (existing
...
- [ ] 4.2 Run `spike/axis/scenario.py`: exit 0 with all five verdicts
      validated against the shipped package
- [ ] 4.3 Full framework suite green
      (`PYTHONPATH=$PWD .venv pytest tests/ -q`); confirm
      `solid_node/node/` and `solid_node/test.py` are untouched by
      `git status`
- [ ] 4.4 Records (coordinator): sync/archive the change, architecture
