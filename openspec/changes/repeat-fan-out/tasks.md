...
- [ ] 0.2 Record the FULL SUITE at the base:
...
- [ ] 0.3 Re-run the four proposal probes and paste their current output
...
- [ ] 1.1 **Fixtures.** `Bead`, a rigid leaf declaring one `Prismatic`
  ...
  `count = Count(9, min=1)`.
- [ ] 1.2 **A copy reads its position.** The four copies of `Column.beads`
  read `index` `0, 1, 2, 3` in order. RED.
- [ ] 1.3 **The position is not identity.** The four share one `uniq_id`,
  ...
  four.
- [ ] 1.4 **A zero repeat still carries nothing.** `.repeat(0)` realizes
  no copies and raises nothing.
- [ ] 1.5 **A repeat of a class that declares `index` is refused**, at
...
- [ ] 1.6 **`Halves` still realizes.** The abacus's real shape is NOT
...
- [ ] 1.7 **A copy's `index` does not become a child name or a tree
...
- [ ] 2.1 **A repeated driven end is n relations.** `Column` states
...
- [ ] 2.2 **A repeated NODE end is the copies' one joint.**
...
- [ ] 2.3 **A repeat one level down.** A root states
...
- [ ] 2.4 **A plain child under the copies.** `yaw.drives(legs.femur.lift)`
  where `legs` is repeated and `femur` is a plain child of `Leg`: one
  fan-out, six bindings.
- [ ] 2.5 **Two repeats in one path are refused** at class definition,
  naming BOTH repeated segments. RED.
- [ ] 2.6 **A list-held child is still refused**, with its own message
...
- [ ] 2.7 **A repeated SOURCE is refused at class definition**, in all
...
- [ ] 2.8 **A broadcast in a formula is refused**: `bad = beads.travel -
  earth` raises at class definition naming the formula and the path.
- [ ] 2.9 **A path that stops on a repeat of a multi-coordinate joint**
...
- [ ] 3.1 **The law is called once per copy, with the copy.**
...
- [ ] 3.2 **The law's signature is unchanged.** The same two-argument
...
- [ ] 3.3 **`ratio=`/`offset=` broadcast unchanged**, resolved once
...
- [ ] 3.4 **Declaration order, copy order.** A class declaring relation
...
- [ ] 3.5 **A named broadcast reads as a tuple of records** off the
...
- [ ] 3.6 **A zero-count broadcast binds nothing and refuses nothing.**
  Simulate; the source keeps the value the author bound and no error is
  raised.
- [ ] 3.7 **Omitted copies are still driven.** A `render()` that omits
...
- [ ] 3.8 **The second run re-solves.** Three successive instants: each
...
- [ ] 3.9 **Symbolic values pass through.** With nothing bound, each
...
- [ ] 4.1 **A broadcast is never inverted.** `earth` unbound, one copy's
...
- [ ] 4.2 **A doubly bound copy names that copy.** The author binds
...
- [ ] 4.5 **`get_coordinate`** in `tests/test_ports.py`: every name
...
- [ ] 4.6 **The pair is symmetric.** For every reported name,
...
- [ ] 5.1 `solid_node/node/declarative.py`:
...
- [ ] 5.2 `solid_node/motion/couplings.py`: `BroadcastRef` beside
  ...
  `BroadcastRef`.
- [ ] 5.3 `Relation.resolve` returns a LIST of records; the law callable
...
- [ ] 5.4 `_step_relation` never reads a broadcast record backwards, and
  `_refuse` raises `NotInvertible` with the broadcast reason. Do not add
  an error kind.
- [ ] 5.5 `solid_node/motion/ports.py`: `get_coordinate`, checked against
...
- [ ] 6.2 **Pose comparison over every project that uses `.repeat()` with
...
- [ ] 6.3 Note in `evidence.md` which of the fourteen are DEFERRED at
...
- [ ] 6.4 A pose comparison run over a project whose repeat is WIRED
...
- [ ] 7.1 Sync the three delta specs into `openspec/specs/` and archive
  the change, in the order the framework-change skill states.
- [ ] 7.2 A new ADR under `docs/adrs/NODE/`: *A relation broadcasts over
  ...
  chronological order.
- [ ] 7.3 `docs/architecture.md` §Couplings: the broadcast, in the
  reference voice.
- [ ] 7.4 `docs/driving.rst`: a fan-out passage with the abacus's
  `for` loop beside the sentence that replaces it, and the source
  refusal stated.
- [ ] 7.5 `docs/declaring.rst`: the paragraph "Per-unit variation never
...
- [ ] 7.6 `docs/api-reference.rst`: `get_coordinate` beside
  `declared_ports`; `docs/changelog.rst` `Unreleased`.
- [ ] 7.7 `tests/test_docs_exports.py` passes unchanged — confirm rather
  than assume.

## 8. The plan note and the findings

- [ ] 8.1 `workflow/warts.md`: mark FIXED the fan-out finding in its four
...
- [ ] 8.2 `workflow/docs/motion-catalogue-2.md` §3.1 was refined when
...
- [ ] 8.3 Do NOT touch `libresolid-studio/docs/motion-general-refactor.md`
  or any project. OpenCycloid's and the abacus's stage B are their own
  cycles in their own repositories.

## 9. Evidence

- [ ] 9.1 `evidence.md`: the base and head suite counts; the RED text of
...
- [ ] 9.2 **Pixels.** `solid snapshot` on a small bench whose four
...
- [ ] 9.3 The three proposal probes re-run against the built tree, with
  their new output recorded beside the old.

## 10. Open questions to close or carry

- [ ] 10.1 Carry: a `count` on the copy. Closed for now for want of a
...
- [ ] 10.2 Carry: indexing a repeat in a class body (`beads[2].travel`),
...
- [ ] 10.3 Close or carry: whether a copy should learn its index DURING
...
- [ ] 10.4 Close: the wiring keyword on `.repeat()`. Not built, with the
