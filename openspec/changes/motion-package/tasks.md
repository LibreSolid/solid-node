...
- [ ] 1.1 Write `tests/test_motion_package.py`: `solid_node.motion.ports`
...
- [ ] 1.2 In the same file, assert `solid_node.motion.joints` and
  ...
  n.startswith('_')] == []`).
- [ ] 1.3 In the same file, assert `solid_node.motion` itself exports no
  port, time-base, joint or coupling name: reading `RotationalPort` off
  the package raises `AttributeError`.
- [ ] 1.4 In the same file, assert the refusal: `from solid_node.node
...
- [ ] 1.5 In the same file, assert both import orders in fresh
...
- [ ] 1.6 In the same file, assert import cost: a fresh interpreter that
...
- [ ] 2.3 Create `solid_node/motion/joints.py`: SPDX header and a
...
- [ ] 2.4 Create `solid_node/motion/couplings.py`: SPDX header and a
...
- [ ] 2.5 Delete `solid_node/node/ports.py` and
  `solid_node/node/timebase.py`.
- [ ] 2.6 Confirm the new package ships: check `pyproject.toml` /
...
- [ ] 3.1 Remove `Port`, `RotationalPort`, `TranslationalPort`,
  `SignalPort`, `declared_ports` and `Time` from `_EXPORTS` in
  `solid_node/node/__init__.py`.
- [ ] 3.2 Add the `_MOVED` table (the six names plus `ports` and
...
- [ ] 3.3 Update the module docstring of `solid_node/node/__init__.py`:
...
- [ ] 5.2 `docs/api-reference.rst`: repoint every autodoc target that
...
- [ ] 5.3 `grep -n "solid_node.node.\(Port\|Rotational\|Translational\|Signal\|declared_ports\|Time\)" docs/*.rst`
  returns nothing.
- [ ] 5.4 `docs/declaring.rst` (or whichever page the
  ...
  and drivers.
- [ ] 5.5 `docs/changelog.rst`: an "Unreleased" entry named for the
...
- [ ] 5.6 Docs evidence. The workspace venv has no `sphinx-build`
...
- [ ] 6.1 Write `docs/adrs/NODE/ADR-087-one-module-one-question.md`
  ...
  location only**.
- [ ] 6.2 `docs/adrs/README.md`: add the ADR-087 row in the NODE table
  in number order, with its amends note; update the ADR-056 and ADR-072
  rows' amended-by fields.
- [ ] 6.3 `docs/architecture.md`: the Node model section's authoring
...
- [ ] 6.4 `openspec validate --change motion-package` passes, and the
  four delta specs still match the baseline requirement text they
  modify.

## 7. Project evidence

- [ ] 7.1 In `/home/asa/devel/libresolid-studio/projects/3DPrintedClocks`,
...
- [ ] 7.2 Run that project's `simulation/shared` and
...
- [ ] 8.2 In the same file's project table, fill 3DPrintedClocks' "Paths"
