...
- [ ] 0.2 **Cycle 1 must be on the branch.** Confirm `repeat-fan-out`
...
- [ ] 0.4 Re-run every probe in `evidence/` at the base and paste its
...
- [ ] 0.5 **Capture the BASE poses of all 23 motion-layer projects**,
...
- [ ] 0.6 **Capture the SECOND-RENDER base evidence for Prusa i3 and
...
- [ ] 0.7 Read `evidence/survey.md`. It is the empirical basis of the
...
- [ ] 1.1 **A chain stated one level down solves.** A movement stating
...
- [ ] 1.2 **An ancestor sources from a descendant-solved coordinate.** A
...
- [ ] 1.3 **The refusal names the class and the path.** The same tree with
...
- [ ] 1.4 **A deferred relation moves a body the walk has not read yet.**
  ...
  after `assemble()`. RED.
- [ ] 1.5 **A wiring whose source a descendant solves binds.** RED with the
  wiring's unbound-source error.
- [ ] 1.6 **A broadcast copy defers and refuses one by one.** A broadcast
  ...
  today).
- [ ] 1.7 **A contradiction is still refused where it is stated.** An
...
- [ ] 1.8 **Each phase runs once per enumeration.** A three-level tree
...
- [ ] 2.1 **A class reading its own derived coordinate is refused.** Thor's
...
- [ ] 2.2 **A class reading a coordinate its own relation binds is the
  same refusal.** openflexure's shape: `steps.drives(rotor.spin,
  ratio=0.5)` with `self.rotor.spin.value` read in `simulate()`. RED.
- [ ] 2.3 **An ancestor reading a coordinate a descendant's relation binds
  is refused**, naming both classes. RED.
- [ ] 2.4 **A rest-default guard is NOT refused.** Prusa's shape: the
...
- [ ] 2.5 **A read of a coordinate nothing ever binds is not refused by
  this rule**: it stays the unreached refusal, by its own name.
- [ ] 2.6 **The message carries the read's source location.** Assert the
  file and line of the reading statement appear in the message.

## 3. Red first: the stale author-bound joint

- [ ] 3.1 **A rest-default joint stands where it says it stands.**
...
- [ ] 3.2 **A NON-ZERO default for the hangprinter's shape**, where the
...
- [ ] 3.3 **A binding made outside any phase is not cleared.** Construct a
...
- [ ] 3.4 **A coordinate reads its bound value between enumerations.**
...
- [ ] 3.5 **Two assemblies animating one node clear only their own.**
  GREEN today; must stay green.

## 4. Red first: a subclass replaces a named relation

- [ ] 4.1 **The replacement.** `evidence/probe_subclass.py` as a test: the
...
- [ ] 4.2 **The base is unaffected**: it still enumerates and solves its
  own, and an instance of the base is unchanged.
- [ ] 4.3 **The position is the base's.** Base declares `a`, `drive`, `b`;
  the subclass replaces `drive`; the enumeration order is `a`, `drive`,
  `b`. RED.
- [ ] 4.4 **A bare statement stays additive**, and so does a named
  relation whose name no base used. GREEN today; must stay green.
- [ ] 4.5 **Reading the name.** Off the subclass: the replacing
...
- [ ] 5.1 **A descendant's `simulate()` reads what an ancestor's relation
  ...
  31 coordinate reads.
- [ ] 5.2 **Nothing that solves today is deferred.** Instrument the solver
...
- [ ] 5.3 **The pass order is unchanged** for a tree that defers nothing:
  ...
  reordering.
- [ ] 5.4 **A descendant simulates against the snapshot just bound.**
...
- [ ] 6.1 `solid_node/node/phase.py`: the enumeration's own stack beside
  the phase stack — open, current, close — and the record of unbound
  reads (§5 of `design.md`).
- [ ] 6.2 `solid_node/node/assembly.py`: `_lifecycle_render` opens the
  ...
  enumerate once.
- [ ] 6.3 `solid_node/motion/couplings.py`: `solve_relations` becomes the
...
- [ ] 6.4 `solid_node/motion/ports.py`: `bind` records the slot on the
...
- [ ] 6.5 Nothing in `joints.py`, `declarative.py`, the serializer or the
  CLI changes. If one of them has to, STOP and report before editing it.
- [ ] 6.6 Every error message states the order in the words the design
  uses, names the class that stated the relation, and names a deferred
  relation as deferred.

## 7. The suite

- [ ] 7.1 `.venv/bin/python -m pytest -x -q` from the worktree. Exact
  counts against 0.3. Every pre-existing failure shown to be pre-existing.
- [ ] 7.2 Run `tests/test_couplings.py`, `tests/test_kinematics.py`,
...
- [ ] 8.1 `capture_poses.py` wall time on Thor, 3DPrintedClocks wall clock
...
- [ ] 8.4 A regression worse than 5% on 8.1 is REPORTED to the pilot in
...
- [ ] 9.1 **Pose comparison over all 23 projects, base against head**,
...
- [ ] 9.2 **Prusa i3 and hangprinter: the second render goes to 0.** The
...
- [ ] 9.7 Every overlay is thrown away after its capture. Nothing under
  `projects/` is written at any point; confirm with `git status` in each
  project touched.

## 10. The record

- [ ] 10.1 Write the ADR: the enumeration's simulate phases are one tree
...
- [ ] 10.2 `docs/adrs/README.md`: the new row, in order.
- [ ] 10.3 `docs/architecture.md`: rewrite the paragraph describing the
  simulate phase and the solver's boundary, rather than appending a note.
- [ ] 10.4 `docs/driving.rst`: the Relations section states where a
  ...
  Unreleased.
- [ ] 10.5 `workflow/warts.md`: mark the four findings and the rider FIXED
  under this cycle's name, in the shape the earlier entries use, and leave
  the finding history intact.

## 11. Close

- [ ] 11.1 `openspec validate --strict whole-tree-fixpoint`.
- [ ] 11.2 Sync the four delta specs into the baselines and archive the
  change, only after the orchestrating session's review.
- [ ] 11.3 Report: the two order-of-events tables as built, the suite
