...
- [ ] 1.1 Fixture helpers: a `brep(shape, name)` writing and loading a cached
...
- [ ] 1.2 **The between-plates scenario.** A thin disc between two plates
...
- [ ] 1.3 **Containment.** A small cube wholly inside a big cube: the boolean
...
- [ ] 1.4 **Cavity.** A small cube inside a hollow cube's void, touching
  ...
  `TopAbs_IN`.
- [ ] 1.5 **Flush contact.** The existing 2×2×2 boxes at exactly 2.0 offset
...
- [ ] 1.5a **The classifier is loaded solid by solid.** Assert the guard
...
- [ ] 1.6 **Margin.** Two solids whose faces are separated, in solid 1's
...
- [ ] 1.7 **A compound straddling the partner.** Shape 1 a compound of two
...
- [ ] 1.8 **Both directions are needed.** Two tests, each of which fails if one
...
- [ ] 1.9 **Declines.** A faceless shape, a shape with no solids, a solid with
  ...
  decline may raise.
- [ ] 1.9a **The record carries the local shape.** A `_place_solid` record's
  ...
  tree.
- [ ] 1.10 **The face-box cache.** `cached_face_boxes` is filled once per
...
- [ ] 1.11 **Face boxes are conservative on a curved face carrying a
...
- [ ] 1.12 In `tests/test_exact_geometry.py`, the assertion-level scenarios:
  ...
  in that module.
- [ ] 1.13 `assertAssemblySupported` still reaches the same support graph on an
...
- [ ] 1.14 Every existing test in `tests/test_exact_geometry.py`,
...
- [ ] 2.1 `solid_node/exact.py`: `_face_box_cache = {}` beside `_bounds_cache`,
...
- [ ] 2.2 `solid_node/exact.py`: `cached_face_boxes(shape)`, shaped exactly
...
- [ ] 2.3 `solid_node/test.py`: `_FACE_BOX_MARGIN = 1e-6` and
...
- [ ] 2.4 `solid_node/test.py`: `_faces_disjoint(shape1, placed1, shape2,
...
- [ ] 2.5 `solid_node/test.py`: the containment guard, as a small helper beside
  ...
  nothing new).
- [ ] 2.5b `_place_solid`: append the solid's LOCAL shape as the ninth field
...
- [ ] 2.6 `_placed_intersection`: call the tier as the first thing inside the
...
- [ ] 2.7 `_exact_verdict`: call the tier after the existing `_boxes_disjoint`
...
- [ ] 2.8 Confirm by reading — and state in the report — that
...
- [ ] 3.1 `tests/test_face_box_culling.py` green.
- [ ] 3.2 `tests/test_exact_geometry.py` and `tests/test_intersection_memo.py`
  green, the pre-existing tests unedited.
- [ ] 3.3 `tests/test_broad_phase_culling.py`, `tests/test_adaptive_broad_phase.py`,
  `tests/test_assembly_supported.py` and `tests/test_sparse_statics.py` green.
- [ ] 3.4 `tests/test_assertions.py`, `tests/test_assembly_integrity.py`,
...
- [ ] 3.5 The whole suite green, including a faceted-kernel run, and record in
...
- [ ] 4.1 `docs/adrs/TEST-FRAMEWORK/ADR-092-face-boxes-decide-an-enclosed-pair-without-a-boolean.md`,
...
- [ ] 4.2 `docs/adrs/README.md`: the row in the TEST-FRAMEWORK section in
  chronological order, and "extended by 092" on the rows of ADR-029 and
  ADR-091.
- [ ] 4.3 `docs/architecture.md`: the exact-path sentences of the shared
...
- [ ] 4.4 `docs/testing.rst`: one paragraph in the assembly-integrity section
...
- [ ] 4.5 `docs/changelog.rst` "Unreleased".
- [ ] 4.6 `workflow/warts.md`, the wall-clock-02 section's THIRD bullet: mark
...
- [ ] 4.7 `workflow/warts.md`, a NEW bullet in the same section: **An exact
...
- [ ] 4.8 Update the last bullet of that section if it still lists this cycle
...
- [ ] 5.1 **What the tier decides, at one instant.** At the declared default
...
- [ ] 5.2 **Which solids remain.** For the same instant, record the names of
...
- [ ] 5.3 **The tier's own cost.** Time `_faces_disjoint` itself
...
- [ ] 5.4 **The swing sweep, end to end, after.** `@testing_steps(48)`, exact
...
- [ ] 5.5 Confirm the test's verdict is identical before and after — same
  passes, same failures, same messages — and say so in `evidence.md`.
- [ ] 5.6 Write `openspec/changes/face-box-broad-phase/evidence.md`: the
...
- [ ] 6.1 `openspec validate face-box-broad-phase --strict` passes.
- [ ] 6.2 Sync the modified `test-framework` and `exact-geometry` baseline specs
