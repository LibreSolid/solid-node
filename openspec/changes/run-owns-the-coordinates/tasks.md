...
- [ ] 0.3 Re-read `design.md` §5–§7 against the tree: confirm by a probe
...
- [ ] 1.1 `parts.py`: `Arbor(Solid2Node)` with `turn = Revolute(axis=(0, 0, 1), unit='deg')`;
  `Slide(Solid2Node)` with `travel = Prismatic(axis=(1, 0, 0), unit='mm')`;
  `Wheel(Solid2Node)` with a plain `turn = RotationalPort(unit='deg')`.
- [ ] 1.2 `machine.py`: `tooth_window(source, target)` returning
...
- [ ] 1.3 `machine.py` continued: `Backwards` (running; `crank.drives(first.turn, ratio=2.0)`,
...
- [ ] 1.4 Confirm every fixture except the refusal fixtures poses
...
- [ ] 2.1 (1) `Time.running()` is accepted on a root: `mode == 'running'`,
...
- [ ] 2.2 (2) `Sim(Train(), 0.1).state` has exactly the keys `crank`,
...
- [ ] 2.3 (3) two `move('crank', by=10, duration=1.0)` calls, each run to
...
- [ ] 2.4 (3b) the kink case: `move('lever', by=40, duration=0.8)`, dt 0.1,
...
- [ ] 2.5 (4) `Backwards`: `move('crank', by=10, duration=1.0)` →
  `first.turn == 20`, `second.turn == 5`.
- [ ] 2.6 (5) `Train`: after 2.3, `slide.travel` and `lever` unchanged on
  every tick (assert inside an `every(0.1, ...)`).
- [ ] 2.7 (6) `Differential`: `move('wrist_in', by=10, duration=1.0)` then
...
- [ ] 2.10 (9) `reset()` → `snapshot() == initial`, tick 0, state equals
  the rest pose, `commands == ()`, `trajectory == []`.
- [ ] 2.11 (10) `trigger('Advance')` twice (each run to completion) →
...
- [ ] 2.12 (11) `rate('crank', 90.0)` then `move('crank', by=10, duration=1.0)`
  ...
  active is a no-op.
- [ ] 2.13 (12) after ten ticks: `node.render()`, reading every
  ...
  from the same bank.
- [ ] 2.14 (13) `Sim(Stepped(), 0.1)` raises `UnsupportedLaw` naming the
  ...
  untimed unchanged.
- [ ] 2.15 reverse refused: `move('crank', by=-10, ...)`,
...
- [ ] 2.16 `Ranged`: moving the crank so `first.turn` would reach `100`
  raises `JointRangeError` on the crossing tick naming `first.turn`,
  `100`, the range; bank at the last admitted tick.
- [ ] 2.17 `Unbound`: `Sim(Unbound(), 0.1)` refused naming the coordinate;
...
- [ ] 2.18 recording: default `trajectory == []` after 10 000 ticks and
...
- [ ] 2.19 `set_state` coordinate ids: on `Train()` (no `Sim`),
  ...
  through the joint.
- [ ] 2.20 couplings: a run-bound slot survives `clear_solved` across
...
- [ ] 2.21 time base plumbing: `animation_block` of a running root has no
  `loop`; `manager/snapshot` keyframes the fraction; unbound `time`
  reads `$t` on root and child, `set_keyframe(2.5)` reads `2.5`.
- [ ] 2.22 import cost, in a fresh interpreter (the shape of
  ...
  `cadquery`.
- [ ] 2.24 `Follower`: over ten ticks of `move('crank', by=10, duration=1.0)`,
...
- [ ] 2.25 serializer: a root declaring a `by=` instruction beside a
...
- [ ] 2.23 (14) the existing suites are the regression net: nothing in
...
- [ ] 3.1 `motion/ports.py`: `Time.loop` optional (`None` for running),
...
- [ ] 3.2 `node/assembly.py::read_time`, `core/serializer.py::animation_block`,
  `manager/snapshot.py`: `loop is None` is no loop. Greens 2.21.
- [ ] 3.3 `motion/ports.py::bind`: refuse a binding of a run-bound sink by
...
- [ ] 3.4 `motion/couplings.py`: `ResolvedEnd.bound()` returns `True` for a
  ...
  2.20 with 4.x.
- [ ] 3.5 `node/assembly.py::set_state`/`_receive_state`/`_undo`: under a
...
- [ ] 4.1 `simulation/program.py`: `compile_program(root, bank_ids)` over
  ...
  half of 2.2.
- [ ] 4.2 `simulation/run.py`: `Run` holding the bank, the program, one
  ...
  2.13, 2.15, 2.16, 2.18.
- [ ] 4.3 `simulation/sim.py`: `state=` and `record=` arguments; the
...
- [ ] 4.4 `simulation/instruction.py`: `Instruction(targets=None, duration=None, *, by=None)`,
...
- [ ] 4.5 `simulation/__init__.py`: `_EXPORTS` gains `RunConflict` (run)
  and `UnsupportedLaw` (program), lazily. `docs/api-reference.rst`
  autoclass entries. Greens 2.22's positive half.

## 5. Evidence and documentation

- [ ] 5.1 `evidence.md`: the base suite counts (0.2), the probe outputs
  ...
  against.
- [ ] 5.2 `docs/animation.rst`: the third base, what `self.time` reads under
...
- [ ] 5.3 `docs/architecture.md`: the Simulation section gains the running
...
- [ ] 6.1 Every case of section 2 green; every suite of 2.23 green with
  the same counts as 0.2 plus the added cases; no `FutureWarning` added.
- [ ] 6.2 `openspec validate run-owns-the-coordinates --strict` passes.
- [ ] 6.3 Hand the implementation report to the adversarial review with
...
- [ ] 7.1 Extract the three ADRs design.md §13 names, under the next free
  numbers, each citing this change; update `docs/adrs/README.md`.
- [ ] 7.2 Sync the delta specs into the baselines
  (`openspec-sync-specs`) and archive the change.
- [ ] 7.3 Record in `workflow/open-run-simulation/roadmap.md` that cycle 1
