...
- [ ] 1.1 Add a fixture project whose tree contains one part placed several
...
- [ ] 1.2 Write failing tests asserting a published `viewer.json` carries a
...
- [ ] 2.1 Create `solid_node/core/pieces.py` with the content fingerprint of a
      built artifact (sha256 of file bytes, truncated), cached per
      `(path, mtime)`
- [ ] 2.2 Derive per-piece geometry facts — `size`, `volume`, `watertight` —
      ...
      raising
- [ ] 2.3 Implement the inventory accumulator: register a rigid node with its
...
- [ ] 2.4 Unit-test the accumulator directly: merging across classes, counting
      repeated placements, deterministic ordering, stable ids independent of
      artifact path and walk order

## 3. Publication

- [ ] 3.1 Add the optional `piece_id(node, model)` mapper to
      `serialize_node`, emitting `piece` on rigid nodes and defaulting to no
      piece so existing callers are unchanged
- [ ] 3.2 Publish `pieces` from the builder into `viewer.json` inside the
      existing atomic write, with build-root-relative model references
- [ ] 3.3 Publish `pieces` from `export_node` into `manifest.json` with
      `models/`-relative references, on top of the existing artifact
      deduplication
- [ ] 3.4 Publish `pieces` from the browser snapshot renderer so all three
      producers agree
- [ ] 3.5 Confirm artifact sweeping still preserves every model the inventory
      names, and that an unchanged rebuild republishes a byte-identical document

## 4. Evidence

- [ ] 4.1 Run the framework test suite
- [ ] 4.2 Rebuild `v8-engine` and confirm 24 pieces covering 119 placements with
      correct counts and plausible extents
- [ ] 4.3 Rebuild `gearbox` and confirm 18 pieces, with the six bushing classes
      and the two housing walls each reported as one piece naming every
      contributing source
- [ ] 4.4 Confirm export and build documents agree on piece ids and counts for
      the same model

## 5. Records

- [ ] 5.1 Extract an ADR for content-derived piece identity alongside
      parameter-hashed artifact keys, and update the ADR index
- [ ] 5.2 Update `docs/architecture.md` if the published-document synthesis
      changed
