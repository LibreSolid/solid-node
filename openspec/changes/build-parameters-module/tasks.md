...
- [ ] 1.1 Add the parameter-module cases to `tests/test_node_lazy_exports.py`:
...
- [ ] 1.2 Point `tests/test_declarative_algebra.py` at
      `solid_node.parameters` for the kinds and `DimensionError`. Confirm it
      fails on the import.

## 2. Move the parameter layer

- [ ] 2.1 Create `solid_node/parameters.py` with the module docstring stating
...
- [ ] 2.2 Reduce `solid_node/node/declarative.py` to the structure layer,
...
- [ ] 2.3 Drop the eight parameter entries from `solid_node/node/__init__.py`'s
      export map and say in its docstring where they went.
- [ ] 2.4 Point `solid_node/math.py`'s deferred formula imports at
      `solid_node.parameters`.

## 3. Callers inside the framework

- [ ] 3.1 Update the declarative test project and every test importing a kind
      or a parameter error.
- [ ] 3.2 Run the full suite; it must pass with no import of a removed name.
- [ ] 3.3 Run flake8 over the changed files and compare against the base.

## 4. Documentation

- [ ] 4.1 `docs/declaring.rst`: every example imports the kinds from
      `solid_node.parameters`, and the page states which module answers for
      parameters, node classes and drivers.
- [ ] 4.2 `docs/cli.rst`: the `--set` example's import line.
- [ ] 4.3 `docs/api-reference.rst`: a Parameters section documenting the kinds,
      `Quantity` and `declared_parameters`.
- [ ] 4.4 `docs/changelog.rst` and `HISTORY.rst`: record the breaking move.
- [ ] 4.5 `docs/architecture.md`: the module map gains the parameter module and
      the one-way dependency.
- [ ] 4.6 Build the docs and confirm no new warning or error.

## 5. Record

- [ ] 5.1 Extract ADR-067 for the public-surface boundary; note it on ADR-062;
      update the ADR index.
- [ ] 5.2 Sync the three modified capabilities into `openspec/specs/`.
- [ ] 5.3 Validate, archive the change, and commit the completed record.

## 6. Projects

- [ ] 6.1 Migrate v8-engine, Inmoov-sim and Metamaquina2 on their
