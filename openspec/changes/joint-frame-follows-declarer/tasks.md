...
- [ ] 0.2 **Cycle 1 must be integrated first.** Confirm that
...
- [ ] 0.4 **Capture the BASE poses of all 23 projects now**, before any
...
- [ ] 0.5 Re-read `evidence/survey.md`. It is the empirical basis of the
...
- [ ] 1.1 **A joint through the body's own origin needs no anchor.** A
...
- [ ] 1.2 **One class, several placements, one declaration.** Four copies
...
- [ ] 1.3 **The line turns with the body.** A body placed with
  `rotate(90, [1, 0, 0])` declaring `axis=(0, 0, 1)`: the published
  rotation's axis is `[0, 0, 1]` exactly. RED — today it publishes
  `[0, -1, 0]`.
- [ ] 1.4 **Thor's elbow, in the forearm's own frame.** With the fixture
...
- [ ] 1.5 **A slide runs along the body's own axis.** The gantry's
  ...
  zero.
- [ ] 1.6 **Two instances placed differently resolve identical
...
- [ ] 1.7 **A joint's line survives a change to the rest placement.** The
  same instance, placed, bound, re-placed differently, bound again: the
  joint's own operations are the same both times.
- [ ] 1.8 **A symbolic rest placement no longer refuses a joint.** The
...
- [ ] 1.9 **The normalized axis is snapped.** `axis=(0, 0, 3)` publishes
...
- [ ] 1.10 **An anchor is published as written.** An anchor the project
...
- [ ] 2.1 **`carries` defaults to `(0, 0, 0)` and resolves at
...
- [ ] 2.2 **A defaulted carried point still refuses at binding when it
...
- [ ] 2.3 **An orbit is stated in the body's own frame.** A disk
...
- [ ] 2.4 **The Internal Cycloidal Actuator's algebra still holds.**
...
- [ ] 3.4 The hexapod's fixtures — `test_the_composition_is_the_one_the_hexapod_hand_inverts`,
...
- [ ] 4.1 `tests/joint_project/arm.py`: `Forearm.elbow` becomes
...
- [ ] 4.2 `tests/joint_project/arm.py`: `Arbor.turn` loses its
...
- [ ] 4.3 `Gantry`/`Carriage` keeps its quarter turn: it is now the
...
- [ ] 5.1 `Joint.place` uses `self.axes(node)` and
...
- [ ] 5.2 Delete `Joint._carry`, `_OwnPlacedOrigin`, `_OWN_PLACED_ORIGIN`
...
- [ ] 5.3 Rename the two hooks' meaning in their docstrings without
  ...
  survive intact.
- [ ] 5.4 `Joint.resolve` normalizes the axis and SNAPS each component
  through `_snapped` before returning it. Anchors are not snapped.
- [ ] 5.5 `Orbit.__init__` takes `carries=(0, 0, 0)`; `Orbit.resolve`
...
- [ ] 6.2 FULL SUITE: `.venv/bin/python -m pytest -x -q` from the
  worktree, exact counts in `evidence.md`, matched against 0.3. Any new
  failure stops the cycle.
- [ ] 6.3 **No test of ADR-093, ADR-094 or ADR-095 was edited except the
  ...
  design says.
- [ ] 6.4 Grep the framework for any remaining consumer of the deleted
...
- [ ] 7.1 Rewrite and compare the **20 projects the survey says need no
...
- [ ] 7.2 Rewrite and compare the **three projects whose pose-changing
...
- [ ] 7.3 **3DPrintedClocks wall clock 48 is the named exception.** Its
...
- [ ] 7.5 Record in `evidence.md`, per project: the model list, the pose
...
- [ ] 8.1 `docs/driving.rst`, the joints section: rewrite around the new
...
- [ ] 8.2 `docs/changelog.rst`, Unreleased: the entry, addressed to the
...
- [ ] 8.3 The ADR: a new NODE ADR, "A joint is stated in the frame of
...
- [ ] 8.4 Update `docs/adrs/README.md` (new row, and ADR-088's status
  line) and `docs/architecture.md` where it describes the joint's frame.
- [ ] 8.5 `openspec validate --strict joint-frame-follows-declarer`;
...
- [ ] 8.6 File in `workflow/warts.md`: close the own-placed-origin
