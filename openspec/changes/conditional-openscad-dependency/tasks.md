...
- [ ] 1.1 Add a framework test that builds an all-exact fixture with
...
- [ ] 1.2 Add a framework test that renders a `Solid2Node` fixture with
...
- [ ] 1.3 Add the same red test for `solid develop --openscad`, for
...
- [ ] 2.1 Add a single availability helper resolving the OpenSCAD binary once
      per process and reporting whether it is present, so the check is not
      repeated per node during a build.
- [ ] 2.2 Give it one error type carrying what needed the binary and the
      remedy, so every requiring path raises the same shape.
- [ ] 2.3 Add a framework test that the helper is never consulted during an
...
- [ ] 3.5 Confirm a faceted `FusionNode` reaches 3.1 exactly as a
      `Solid2Node` leaf does, and that an exact one never does.
- [ ] 3.6 Confirm a `JScadNode` reaches NONE of the five checks: its
...
- [ ] 3.7 Turn 1.2 and 1.3 green.

## 4. The default renderer stays put

- [ ] 4.1 Add a test pinning that `solid snapshot` with no `--renderer`
      ...
      project's backends.
- [ ] 4.2 Add a test that the OpenSCAD renderer, when unavailable, does not
      produce an image through the web renderer.

## 5. Records and documentation

- [ ] 5.1 Update `README.rst` to state OpenSCAD as conditional on the backends
      and commands that use it, naming them, rather than as a blanket
      requirement. Do not imply OpenSCAD is deprecated.
- [ ] 5.2 Supersede ADR-004 in part: the universal-compilation-target clause
...
- [ ] 5.3 State the deferral explicitly in the ADR for this cycle, under
      ...
      follow-on.
- [ ] 5.4 Update `docs/architecture.md` where the synthesis changed.

## 6. Validating callers

- [ ] 6.1 Run the full framework suite. All green.
- [ ] 6.2 Build, test and snapshot `projects/snowman` with `openscad` removed
      from the PATH. The build and tests must succeed; the snapshot must fail
      naming `--renderer web`, and must succeed when given it.
- [ ] 6.3 Build `projects/snowman-3` (Solid2) with OpenSCAD present and
      confirm nothing changed. Then remove it from the PATH and confirm the
      failure names the node and its backend.
- [ ] 6.4 Build one CadQuery project with OpenSCAD present
