...
- [x] 1.1 **Fixtures.** A `Carried` node class declaring
...
- [x] 1.2 **The attitude is untouched.** Bind the orbit at θ ∈ {0, 17,
...
- [x] 1.3 **The carried point lands where the rotation says.** At the
  ...
  the STATED one. RED.
- [x] 1.4 **One operation, and it is a translation.** `serialized(node)`
  is exactly one `['t', [...]]` per binding; re-binding replaces it and
  leaves one. RED.
- [x] 1.5 **The default carried point is the placed origin, exactly.**
...
- [x] 1.6 **The anchor may be any point of the line.** Same axis, two
...
- [x] 1.7 **A carried point on the axis is refused by name.** A body
...
- [x] 1.8 **The argument refusals are realization-time.** `carries` of
  ...
  `at` takes. RED.
- [x] 1.9 **A symbolic binding publishes the trigonometry.** Bind the
...
- [x] 1.10 **A relation drives an orbit.** `shaft.turn.drives(disk.orbit)`
...
- [x] 1.11 **A range on an orbit.** `range=(-90, 90)`: `170` raises
  ...
  the name.
- [x] 1.12 **The declaration is exported and enumerable.** `Orbit` is in
...
- [x] 2.1 Add an `Orbit` to the cycle-1 composition fixture — a class
...
- [x] 2.2 **Contiguity with an orbit in the middle.** A `Prismatic`, then
...
- [x] 2.3 **Re-binding the orbit of several joints** returns it to its
  own slot, leaving one operation. RED.
- [x] 2.4 The whole of the existing cycle-1 section stays green with no
...
- [x] 3.1 `solid_node/motion/joints.py`: generalize
...
- [x] 3.2 `solid_node/motion/joints.py`: add
...
- [x] 3.3 `solid_node/motion/joints.py`: `Joint.resolve` resolves a
...
- [x] 3.4 `solid_node/motion/joints.py`: the `Orbit` class.
...
- [x] 3.5 `solid_node/motion/joints.py`: `__all__` gains `'Orbit'`. The
...
- [x] 3.6 The on-axis refusal's error class: raise the module's existing
...
- [x] 3.7 Run tasks 1 and 2. Everything green. Then the whole suite:
...
- [x] 4.1 `docs/adrs/NODE/ADR-094-<slug>.md`: "An orbit carries a point,
...
- [x] 4.2 `docs/adrs/README.md`: the ADR-094 row in the NODE section in
  chronological order, and "extended by 094" on ADR-088's row.
- [x] 4.3 `docs/architecture.md` §Joints (the passage beginning "**Joints**
...
- [x] 4.4 `docs/api-reference.rst`, the Joints section: an
...
- [x] 4.5 `docs/driving.rst`, the joints passage (around lines 191-230):
  ...
  a reference.
- [x] 4.6 `docs/changelog.rst`, `Unreleased`: one entry at the top, in
  ...
  extending ADR-088.)".
- [x] 4.7 Sync the delta into `openspec/specs/joints/spec.md`.
...
- [x] 5.1 `workflow/docs/composed-joints.md`: mark §5 taken up by this
...
- [x] 5.2 `workflow/warts.md`: mark the carried-body sightings fixed by
...
- [x] 5.3 `libresolid-studio/docs/motion-general-refactor.md` is SHOP
...
- [x] 6.1 **The Internal Cycloidal Actuator's algebraic identity, as a
...
- [x] 6.2 **YouCanBuildDog's `swung_offset`, as a second fixture.** Its
  ...
  nothing.**
- [x] 6.3 **No pose comparison of the catalogue is required.** Nothing
...
- [x] 6.4 **Pixels.** `solid snapshot` on a fixture carrying an orbit and
...
- [x] 6.5 **Re-run the design's serialization probe against the real
...
- [x] 7.1 **The declaration order the four projects wrote.** Three of the
...
- [x] 7.2 **The V8's `at`.** Its proposal writes
...
- [x] 7.3 **The export target** (question 6 of the plan note) stays open
