...
- [x] 1.1 Red: `tests/test_time_base.py` — `Time(loop=43200)` bound as
...
- [x] 1.2 Red: unbound `self.time` on the declaring root is a symbolic
...
- [x] 1.3 Red: `set_keyframe(2700)` on the declaring root binds `2700.0` on
...
- [x] 1.4 Red: a child class declaring `Time` linked under an undeclared
...
- [x] 1.5 Implement `Time` in `solid_node/node/timebase.py` (frozen
...
- [x] 1.6 Implement the shared reader in `solid_node/node/assembly.py`:
...
- [x] 1.7 Run the kinematics, declarative-nodes, simulation and flexible-leaf
      ...
      NOTE: a first cut linked a parent's rendered children inside the
      lifecycle wrapper so the reader's walk would work under a bare
      `render()`; `tests/test_declarative_render.py` pins that a bare
      `render()` links nothing, so that was reverted. The walk relies on
      the walkers' links (design D2, ADR-072), and the tests here go
      through `assemble()`.

## 2. Published documents

- [x] 2.1 Red: `export_node` of a declaring root writes
      ...
      current one.
- [x] 2.2 Red: the builder's `viewer.json` for a declaring root carries
      `animation.loop`; for an undeclared root it does not (extend the
      existing `_write_viewer_snapshot` tests).
- [x] 2.3 Red: the browser snapshot's staged document carries
      `animation.loop` under the same rule.
- [x] 2.4 Implement one helper (`animation_block(root, fps, frames)` beside
...
- [x] 3.1 Red: `manager/snapshot.py`'s node preparation keyframes a
      declaring root at `time * loop` for `--time 0.5` and an undeclared
      root at `0.5`; the option's validation is unchanged.
- [x] 3.2 Implement the conversion in `_load_and_prepare_node`. Green.
- [x] 3.3 Red: a `TestCase` over a declaring root decorated
...
- [x] 4.1 Migrate `projects/3DPrintedClocks/design/wall_clock_01`:
...
- [x] 4.2 Run `solid test` and `solid build` for the clock from this
      ...
      EVIDENCE (2026-09-05): `solid test --set facing=0` from the worktree:
      `Ran 4 tests in 214.53 seconds: 4 passed, 0 failed` — the same four
      tests, now sweeping `end=1.5` (one beat) and `end=3.1*3600` (one turn
      of the great wheel) in seconds; `solid build` publishes
      `animation.loop == 43200.0` (see the build log in the cycle report).

## 5. Records

- [x] 5.1 `docs/animation.rst`: a "Declaring the time base" section
...
- [x] 5.2 ADR "Declared time base" in `docs/adrs/NODE/` extending ADR-008
      (mark 008 extended by it), indexed in `docs/adrs/README.md`;
      `docs/architecture.md` kinematics and export sections rewritten for
      the new state.
- [x] 5.3 Full framework suite green; `openspec validate`; sync and archive
      the change.
