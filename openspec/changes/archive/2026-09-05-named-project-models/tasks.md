...
- [x] 1.1 Add a scratch-project fixture, on the pattern of
...
- [x] 1.2 Red (loader): `discover_project` reports both names, their
...
- [x] 1.3 Red (loader): the selection step returns `<build root>/<name>`
...
- [x] 1.4 Red (build): building each model publishes `viewer.json` under its
      ...
      `.lock` files.
- [x] 1.5 Red (cli): `solid build <name>` and `solid build` (default) run the
...
- [x] 1.6 Red (test): `solid test --all` runs both models' companion tests as
      one reported run, each built in its own directory.
- [x] 1.7 Red (models): `solid models` and `--json` list both models in order
...
- [x] 1.8 Guard: the existing single-model fixtures build into the flat root
...
- [x] 2.1 `discover_project` parses the `models` table: name pattern, non-empty,
...
- [x] 2.2 A selection step in the loader maps a reference or none to the
      concrete reference and its build directory; `resolve_node` looks a bare
      word up as a name before reading it as a qualifier.

## 3. Build directories

- [x] 3.1 `Builder` takes the selection's directory; `build_once` carries it
      to the fresh interpreter.
- [x] 3.2 The build-root sweep prunes immediate children that are declared
      model names and spares `.lock` by extension.
- [x] 3.3 `develop` serves the selection's directory; `test`, `snapshot` and
      `export` anchor it before loading the node.

## 4. Commands

- [x] 4.1 `solid_node/manager/models.py`: the `models` command, text and
      `--json`, registered in `cli.COMMANDS` with `needs_node = False`.
- [x] 4.2 `solid build --all`: walk, per-model report, exit status.
- [x] 4.3 `solid test --all`: every declared model as one run.

## 5. Verification

- [x] 5.1 Turn every red and guard test from group 1 green.
- [x] 5.2 Full framework suite.
- [x] 5.3 On the originating project (`projects/3DPrintedClocks`, manifest
      temporarily declaring `wall_clock_01` and `pendulum`, restored after,
      nothing committed there): `solid models`, `solid build --all` (both
      published, 1m17s), `solid build pendulum` and `solid build
      wall_clock_01` (current, documents unchanged), `solid test pendulum`,
      `solid test wall_clock_01 --faceted --set facing=0` (the same two
      6e-13 mm³ tessellation contacts fail on the flat layout with a bare
      reference; not this change), `solid develop wall_clock_01` serving the
      model's own `viewer.json`; the flat `_build` untouched throughout.
      First attempted before the rebase, when the clock could not load on
      base `dbf6798` (its model imports `Time` from `declared-time-base`),
      which exposed the import-failure walk fix; the same steps were also
      run on `projects/v8-engine` with `engine` and `crankshaft`.
- [x] 5.4 On a single-model project (v8-engine): confirm a settled rebuild
      re-derives nothing and every path is unchanged.

## 6. Records

- [x] 6.1 ADR-073 in `docs/adrs/BUILD/`: named project models and per-model build
      directories; amends ADR-005/ADR-024 (reference spellings) and
      ADR-038 (per-directory publication and lock); names the
      originating project.
- [x] 6.2 Update `docs/adrs/README.md` and the build-pipeline and CLI
      sections of `docs/architecture.md`.
- [x] 6.3 `docs/cli.rst` (`models`, `--all`, the fourth spelling),
