...
- [x] 0.3 Record the BASE per-tick cost by re-running cycle 1's own probe,
...
- [x] 0.4 Read `design.md` §1 against `solid_node/scad_expression.py`
...
- [x] 1.1 Laws: `clutch(sources, target)` returning
...
- [x] 1.2 The nested law `alternating(source, target)`: `w = floor(angle / 360)`,
...
- [x] 1.3 The Pascaline-shaped carry, with the fixture's own round
...
- [x] 1.4 The only-jumps laws: `counter(source, target)` returning
...
- [x] 1.5 The awkward laws: `moving_divisor(sources, target)` returning
...
- [x] 1.6 Running roots, each `time = Time.running()`, each with an
...
- [x] 1.7 `Clutch`: `shaft = Driver(default=10.0, unit='deg')`,
  `sleeve = Driver(default=0.0, unit='mm')`, `wheel = Arbor()`,
  `(shaft & sleeve).drives(wheel.turn, law=clutch)`.
- [x] 1.8 `Carry` and `CarryLead`: `column = Driver(default=100.0, unit='deg')`,
  ...
  module's own shape.
- [x] 1.9a `PortDriven`: a running root whose jumping law drives a PLAIN
  ...
  must compile.
- [x] 1.9 `OnlyJumps` (`counter`), `Settled` (`settled`, two drivers),
  ...
  (`crowded`).
- [x] 1.10 Every new fixture POSES UNTIMED at the base, unchanged: a
...
- [x] 2.0 The names do not exist yet: `TooManyCrossings`,
...
- [x] 2.1 **(1) The periodic window gives 76 then 148.** `Sim(Window(), 1/240)`
...
- [x] 2.2 **(1b) The untimed reading is untouched.** `SteppedBody` and
  `WindowBody` pose exactly as at the base at a spread of crank angles,
  including 359.9 and 360.1.
- [x] 2.3 **(2) The clutch.** With `Sim(Clutch(), 1.0)`: moving `shaft`
...
- [x] 2.4 **(3) Three teeth in one tick.** `Sim(Window(), 1/240)` with
...
- [x] 2.5 **(4) A nested jump.** `Sim(Alternating(), 1/240)`: four
...
- [x] 2.6 **(5a) `%` agrees with `floor`.** `Window` and `Remainder`
  stepped side by side through one revolution from `100` read the same
  driven value at EVERY tick.
- [x] 2.7 **(5b) `sign`.** `Reverser` and `Kinked` stepped from `40`
...
- [x] 2.8 **(5c) `wrap`.** `Sim(Wrapped(), 1.0)` moved `500` from `100`
  leaves the driven coordinate at exactly `1000.0`, and two `ceil`
  crossings are recorded.
- [x] 2.9 **(6) The only-jumps refusal.** `Sim(OnlyJumps(), 0.1)`
...
- [x] 2.9a **(6b) A jumping law with nowhere to keep its history.**
  ...
  port's.
- [x] 2.10 **(7) The crossing record.** `Sim(Window(), 1/240, record=4)`
...
- [x] 2.11 **(8) Display-cadence independence.** The same 360° of crank
...
- [x] 2.12 **(9) The large origin.** `Sim(Window(), 1/240, state={'crank': W * 360 + 100})`
...
- [x] 2.13 **(10) The Pascaline-shaped carry.** `Sim(Carry(), 1.0)`
...
- [x] 2.14 **(11) Per-tick cost.** A probe in `evidence/` measuring
  ...
  assertion on `Train`.
- [x] 2.15 **(12) Every existing suite is the regression net.** Nothing
...
- [x] 2.16 The three cases cycle 1 wrote for the refusal change meaning:
...
- [x] 2.17 The compile plan itself: `Sim(Window(), 0.1).program` exposes,
...
- [x] 2.18 The search path: `Sim(NonAffine(), 1.0)` with `a` moved from
...
- [x] 2.19 The two per-tick refusals: `Sim(Crowded(), 1.0)` moved `2000`
...
- [x] 2.20 Snapshot and restore across a crossing: a snapshot taken on
...
- [x] 2.21 A jump in a BACKWARD-solved relation: a root whose law is
...
- [x] 3.1 `_JUMP_CALLS`/`_JUMP_OPERATORS` become the RECOGNIZED set;
  ...
  2.16 and 2.17.
- [x] 3.1a `_relation_edge`: a law whose graph carries a jump and none of
  whose driven ends is a bank coordinate raises `UnsupportedLaw` with
  the message of design §9a. Greens 2.9a.
- [x] 3.2 `_skeleton(root)`: the graph with every jump node replaced by
...
- [x] 3.3 `_argument_graph(node)`: each jump node's level quantity of
...
- [x] 3.4 `_affine_in_sources(graph)`: the structural classification of
...
- [x] 3.5 `_only_jumps(root)`: the continuous skeleton of design §9 —
...
- [x] 3.6 `JumpPlan.increment(start, delta, out)`: the partition of
  ...
  2.18.
- [x] 3.7 `TooManyCrossings(CouplingError)` and the zero-divisor refusal,
  both raised from `JumpPlan.increment` naming the relation, the
  coordinate and the primitive. Greens the raising half of 2.19.
- [x] 3.8 `Edge.increments(values, deltas, crossings=None)`: a graph with
...
- [x] 4.1 `Run`: a second ring beside `self.ring`, the same `maxlen`,
...
- [x] 4.2 `Run.integrate`: `TooManyCrossings` and the zero-divisor
  refusal are caught where `RunConflict` is raised, take `self._refuse(moved)`
  and re-raise, committing nothing. Greens 2.19.
- [x] 4.2a `Run._values()` skips an edge whose targets are all bank keys
...
- [x] 5.2 `docs/scenarios.rst`: the jump bullet LEAVES "What this release
...
- [x] 5.3 `docs/changelog.rst` entry; `docs/architecture.md` Simulation
  section's running paragraph gains the jump plan.
- [x] 5.4 `workflow/open-run-simulation/roadmap.md`: record cycle 2 on
  the branch with its commit, its open questions, and the Pascaline lead
  finding as a note for the module's running migration.

## 6. Green and checked

- [x] 6.1 Every case of section 2 green; every suite of 2.15 green with
  the same counts as 0.2 plus the added cases; no new `FutureWarning`.
- [x] 6.2 The Curta bench's law and the Pascaline module's `handed_on`,
...
- [x] 6.3 `openspec validate integrate-jumps --strict` passes.
- [x] 6.4 Hand the implementation report to the adversarial review with
...
- [x] 7.1 Extract the ONE ADR `design.md` §15 names — how a jump is
  integrated — under the next free number, citing this change; update
  `docs/adrs/README.md`.
- [x] 7.2 Sync the delta spec into the baseline (`openspec-sync-specs`)
  and archive the change.
- [x] 7.3 Record in `workflow/open-run-simulation/roadmap.md` that cycle 2
