...
- [x] 1.1 Write `tests/joint_project/`, in the style of
...
- [x] 1.2 `tests/test_joints.py`, declaration: `Revolute` and
...
- [x] 1.3 The coordinate: reading a joint off an instance yields a
  ...
  `out`.
- [x] 1.4 The frame carry, on Thor's numbers (assert the serialized
  ...
  floats.
- [x] 1.5 The degenerate case, the slide and numeric hygiene: a joint
  ...
  show).
- [x] 1.6 Motion discipline: the joint motion is innermost (before the
...
- [x] 1.7 Symbolic values: binding a joint from an expression in the
...
- [x] 1.8 Range: a numeric binding outside the range raises
  ...
  accepts anything.
- [x] 1.9 Argument resolution: a joint anchored on a declared parameter
...
- [x] 1.10 Wiring: a coordinate wired into a child binds that child on
...
- [x] 1.11 Wiring refusals: a keyword naming no port or joint of the
  ...
  identity.
- [x] 1.12 The published document: build the fixture root, serialize it
...
- [x] 1.13 Import cost: `solid_node.motion.joints` imports the same
...
- [x] 1.14 Run the suite and record the failures: every test in 1.2–1.13
  fails on the tree as cycle 1 left it, for the right reason (the name
  is absent from `solid_node.motion.joints`, not a typo).

## 2. The joints module

- [x] 2.1 `solid_node/motion/joints.py`: SPDX header, module docstring
...
- [x] 2.2 `JointRangeError(ValueError)`.
- [x] 2.3 `Joint`: `__init__(axis, at=(0, 0, 0), range=None, unit=None)`
...
- [x] 2.4 `Revolute` and `Prismatic`: the port kind, the default unit
  (`'deg'` / `'mm'`), and the placement each produces.
- [x] 2.5 `declared_joints(node_class)`: MRO base-first, per-class cache
  like `declared_drivers_of`.
- [x] 2.6 Argument resolution: one function taking the declaration and
...
- [x] 2.7 The frame carry: compose the node's non-motion operations in
  ...
  function.
- [x] 2.8 Placement: `solid_node/node/base.py` gains the way to place an
...
- [x] 2.9 Numeric hygiene: snap carried components within `1e-9` of `0`,
...
- [x] 2.10 Application: `translate(-anchor)`, `rotate(value, axis)`,
...
- [x] 3.1 `solid_node/motion/ports.py`: `declared_ports` also collects a
...
- [x] 4.3 `ChildDeclaration.realize`: unchanged for parameters (wirings
...
- [x] 4.4 Joint arguments resolved at realization: call the resolver
...
- [x] 4.5 `solid_node/node/assembly.py`, `_lifecycle_render`: after
  ...
  coordinate.
- [x] 4.6 Refuse a hand binding of a wired child end from the declaring
...
- [x] 5.1 The user documentation gains the joint: what a joint declares,
...
- [x] 5.2 `docs/api-reference.rst`: autodoc entries for
  `solid_node.motion.joints.Revolute`, `.Prismatic`, `.JointRangeError`
  and `.declared_joints`, beside the Ports section.
- [x] 5.3 `docs/changelog.rst`, "Unreleased": joints, the wiring by
  ...
  change and ADR-088.
- [x] 5.4 Docs evidence: the import check from cycle 1's task 5.6 shape
  — every autodoc target resolves — plus a sphinx build only if the venv
  provides one. Say which was used.

## 6. Records

- [x] 6.1 `docs/adrs/NODE/ADR-088-a-joint-owns-one-coordinate.md`,
...
- [x] 6.2 `docs/adrs/README.md`: the ADR-088 row in the NODE table in
  number order, and the extends notes on the ADR-056 and ADR-066 rows.
- [x] 6.3 `docs/architecture.md`: the Kinematics section gains the joint
  ...
  spec and ADR-088.
- [x] 6.4 `openspec validate joints --strict` passes, and every MODIFIED
  requirement still matches the baseline text it replaces after cycle 1
  synced its own deltas.

## 7. Roadmap

- [x] 7.1 `workflow/motion/roadmap.md`, Progress table, row `2 joints`:
