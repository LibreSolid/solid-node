...
- [x] 0.3 Record the BASE per-tick cost by re-running cycle 1's probe,
...
- [x] 0.4 Probe and paste: that `Joint._span` today REJECTS `None` as a
...
- [x] 1.1 `Ratchet`: a running root with `arbor = Driver(default=40.0,
...
- [x] 1.2 `Swept`: a running root with `steer` driving
...
- [x] 1.3 `Shared`: a running root with `a_in` driving `c.turn` at
  ...
  and not `c.turn`.
- [x] 1.4 `TwoStops`: a running root with two independent groups — a
...
- [x] 1.4b `OpenGate`: a running root with inputs `push`, `crank` and
...
- [x] 1.5 `StopAndJump`: a running root whose `crank` rests at `130`,
  ...
  crank `145`.
- [x] 1.6 `Ranged` rewritten: keep `crank.drives(first.turn, ratio=2.0)`
  and `range=(-90, 90)`, and add a sibling `RangedExact` whose move
  lands `first.turn` on exactly `90`.
- [x] 1.7 `Curved`: a running root whose ranged coordinate is driven by
...
- [x] 1.8 `Lifted`: the shape a bound over a SECOND coordinate would
...
- [x] 1.9 An UNTIMED control for each new fixture that has one, so the
...
- [x] 2.0 The names do not exist yet: `sim.stops`, `Stop`,
...
- [x] 2.1 **(1) Reverse moves are admitted.** On `Train`:
...
- [x] 2.2 **(1b) A reverse rate on an INTEGER input truncates toward
...
- [x] 2.3 **(2) The ratchet, the whole worked example of design §11.**
...
- [x] 2.4 **(3) The swept stop.** On `Swept` at `dt = 0.1` with
...
- [x] 2.5 **(4) Landing exactly on the bound completes.** On
...
- [x] 2.6 **(5) A push into a stop from the stop.** Covered by 2.3's
...
- [x] 2.7 **(6) A multi-source coordinate with one input stopped.** On
...
- [x] 2.8 **(7) Two stops in one tick.** On `TwoStops`, one tick that
...
- [x] 2.9 **(8) A stop and a jump crossing in one tick.** On
  ...
  `0.75`.
- [x] 2.10 **(9) Snapshot and restore across a block.** A snapshot taken
...
- [x] 2.11 **(10) The untimed meaning of an expression range.** The
...
- [x] 2.12 **(11) A stop's record.** With `record=4`, six stops leave
...
- [x] 2.13 **(12) Cadence independence.** The same reverse move of `-10`
...
- [x] 2.14 **(6b) A stop on a NON-AFFINE chain is found by search.** On
...
- [x] 2.15 **(A tick that fails after a stop commits nothing.)** A
  ...
  `refused`.
- [x] 2.16 **(The group is the program's, not the tree's.)** Assert
...
- [x] 2.16b **(The group is who pushes.)** On `OpenGate` with the gate at
  ...
  first case.
- [x] 2.17 **(14) Every existing suite unchanged.** Run
...
- [x] 3.1 `Program.sources`: the inputs reaching each node key, one pass
...
- [x] 3.2 `Edge.affine`: one flag per driven end, from
...
- [x] 3.3 `JumpPlan.cuts(start, delta, ...)`: `_partition` made
  reachable, with no recording, so a stop search can bracket on the
  pieces a jump law already has.
- [x] 3.4 The span table: `compile_program` resolves every banked
...
- [x] 4.2 `_refuse_out_of_range` applies a callable bound to the value
  ...
  evaluated bound.
- [x] 4.3 No import of the simulation layer, asserted by the existing
  import-boundary test if there is one and by inspection otherwise.

## 5. The tick (`simulation/run.py`, `sim.py`, `__init__.py`)

- [x] 5.1 Lift `integrate`'s current body into `_pass(values, deltas,
...
- [x] 5.2 The per-tick span evaluation: each bound evaluated once, at
  the tick's start, from the committed bank.
- [x] 5.3 `_locate`: design §2's three cases, reusing
  `_CROSSING_TOLERANCE`, `_SUBDIVISIONS` and `_BISECTION_ROUNDS` and no
  new tolerance.
- [x] 5.4 The segment loop: detection by the committed value and the
...
- [x] 5.5 `_block`: retire every active command whose input is in the
  union of the stopped groups as `blocked`, with its accumulated
  admitted travel, releasing ownership.
- [x] 5.6 `move` and `rate` lose the reverse refusals;
  `Command._cumulative` truncates toward zero.
- [x] 5.7 The third ring, `Run.stops`, cleared by restore and reset with
...
- [x] 5.8 The crossing fraction rescaled to the TICK when a segment
  records it.

## 6. Evidence and documentation

- [x] 6.1 `evidence/probe_cost.py` re-run and a new
...
- [x] 6.2 `evidence/probe_projects.py` extended: the Pascaline module's
...
- [x] 6.3 `docs/scenarios.rst`: the reverse-move and range bullets leave
...
- [x] 6.4 `workflow/open-run-simulation/design.md` is NOT edited: it is
  the pilot's record, and this change's design is this file's sibling.

## 7. Green and checked

- [x] 7.1 Every case of section 2 green; the full suite green with the
  counts of 0.2 plus the added cases; no new `FutureWarning`.
- [x] 7.2 `openspec validate ranges-are-stops --strict` passes.
- [x] 7.3 Hand the implementation report to the adversarial review with
...
- [x] 8.1 Extract the TWO ADRs `design.md` §18 names — a range is a
...
- [x] 8.2 Sync the delta specs into the baselines (`openspec-sync-specs`)
  and archive the change.
- [x] 8.3 Record in `workflow/open-run-simulation/roadmap.md` that cycle
