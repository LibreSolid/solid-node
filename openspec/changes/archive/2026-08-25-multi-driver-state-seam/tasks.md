...
- [x] 1.1 Red: add a focused test asserting the sweep keys on
      `operation._animator` / `assembly._animated_nodes` (fails while
      the old names exist)
- [x] 1.2 Rename `_driver`/`_driven_nodes` to
...
- [x] 1.3 Green: full existing kinematics/meta test set passes
      unchanged

## 2. Multi-driver state binding

- [x] 2.1 Red: new `tests/test_state_binding.py` covering the six
...
- [x] 2.2 Implement `set_state`/`clear_state` no-op roots on
...
- [x] 3.2 Implement `solid_node/node/ports.py`: `Port` descriptor
...
- [x] 3.3 Implement `connect(source, sink)` on `InternalNode`
      (per-render causal binding, scale applied); export the port
      classes from `solid_node/node/__init__.py`
- [x] 3.4 Green: ports tests pass; whole framework suite passes

## 4. Validation and records

- [x] 4.1 Run the full framework test suite from the worktree bench
      (worktree `.env`, `PYTHONPATH=$PWD`, workspace venv)
- [x] 4.2 Caller validation: run the v8-engine project suite against
      ...
      (v8: 33/33 via `solid test`, verified twice. Spike: its own
      set_state override collided with the API it prototyped and was
      dissolved rather than shimmed — all five verdicts revalidated;
      see spike/FINDINGS.md addendum.)
- [x] 4.3 Update the kinematics spec text and ADR index per the
      ...
      changed
      (Baseline specs synced by archive + manual purpose/animator-body
      correction where the delta's second MODIFIED section header was
      not machine-parsed; architecture.md kinematics section rewritten
      for state binding, animator tag, and ports; no new ADR — the
      decisions live in ADR-056, which stays Proposed; ADR index
      already lists 056.)
