...
- [x] 1.1 **Fixtures.** `FloatingChassis`, a node declaring
...
- [x] 1.2 **The six coordinates enumerate under their dotted names.**
...
- [x] 1.3 **The composition is the hexapod's.** Bind the four the hexapod
...
- [x] 1.4 **The hexapod's own inverse round-trips.** Transcribe
...
- [x] 1.5 **Unbound coordinates are the identity and still read
  ...
  `0.0`, `atol=0`. RED.
- [x] 1.6 **All six unbound places nothing.** A realized
  `FloatingChassis` nothing binds carries NO motion operations at all.
  RED (weakly — record what it distinguishes).
- [x] 1.7 **The binding order of the six does not matter.** Two benches
  ...
  ADR-093. RED.
- [x] 1.8 **Re-binding one coordinate re-places the whole joint.** Bind
  ...
  returns, `atol=0`. RED.
- [x] 1.9 **The anchor.** `AnchoredFloat` bound in `yaw` alone turns
...
- [x] 1.11 **`axis` and `range` are refused.** `Free(axis=(0, 0, 1))` and
  ...
  takes). RED.
- [x] 1.12 **`at` resolves against the instance.** `at` from a declared
...
- [x] 1.13 **A symbolic binding publishes ordinary operations.** Bind the
...
- [x] 1.14 **The declaration is exported and costs no import.** `Free` is
...
- [x] 2.1 **A relation reaches one coordinate by path.**
...
- [x] 2.2 **A relation stated in the joint's own class body.**
  `pose.roll.drives(<something>)` written on the class declaring the
  `Free`, resolved through `OwnRef`. RED.
- [x] 2.3 **The path grammar over more segments.** A `Free` two levels
...
- [x] 2.4 **The joint is not an end.** `tilt.drives(chassis.pose)` and
...
- [x] 2.5 **The node is not an end.** A child whose class declares one
...
- [x] 2.6 **A `Free` cannot be wired, in either role.**
...
- [x] 2.7 **A one-coordinate joint's wiring is untouched.** The existing
...
- [x] 2.8 **A name declared twice.** A class body declaring `pose` as
...
- [x] 2.9 **The dotted binding path.** After a RELATION has bound
...
- [x] 3.1 Add a `Free` to the cycle-1 composition fixture — a class
  ...
  RED.
- [x] 3.2 **Contiguity with a hand-written rotation in the middle.** The
...
- [x] 3.3 **Re-binding one coordinate of the free joint** returns its
  whole run to its own slot, leaving the siblings where they were. RED.
- [x] 3.4 **A `Free` beside an `Orbit`.** Cycle 2's `SpunAndCarried`
  shape with a `Free` declared first: the orbit's single translation
  stays outside it. RED.
- [x] 3.5 The whole of the existing cycle-1 and cycle-2 sections stays
...
- [x] 4.1 `solid_node/motion/joints.py`: `Joint` grows an ordered
  ...
  mis-handles.
- [x] 4.2 `Joint` grows an `axes(node)` hook returning
  ...
  unchanged.
- [x] 4.3 `Free`: the declaration, the six ports, the bound-coordinates
...
- [x] 4.4 `solid_node/motion/ports.py`: `declared_ports` prefers the
  ...
  on the last dot.
- [x] 4.5 `solid_node/motion/couplings.py`: `read_through` gains a branch
...
- [x] 4.6 `solid_node/node/declarative.py`: its local `_coordinate_of`
  ...
  one coordinate.
- [x] 4.7 Run the full suite. Nothing outside the new sections may
  change. Record the before and after counts.

## 5. Specs and decision record

- [x] 5.1 **NOT DONE — deferred to the orchestrator by instruction.** Sync
  the three delta specs into
  ...
  survives verbatim.
- [x] 5.2 ADR-095, `docs/adrs/NODE/ADR-095-a-free-joint-owns-six-coordinates.md`
...
- [x] 5.3 `docs/architecture.md` §Joints: rewrite the "A joint OWNS one
...
- [x] 6.2 `workflow/warts.md`: the hexapod's entry (lines 815-836)
  ...
  is.
- [x] 6.3 Do NOT touch `libresolid-studio/docs/motion-general-refactor.md`
  or any project. The hexapod's stage B is its own cycle in its own
  repository.

## 7. Evidence

- [x] 7.1 `evidence.md`, in the shape cycle 2's has: the full suite
...
- [x] 7.2 **Pixels.** `solid snapshot` on a small bench whose body is
...
- [x] 7.3 **Nothing existing moved.** The framework's own before/after on
  the joint-carrying fixtures of the existing suite, by matrix
  comparison, not by "the tests pass".
- [x] 7.4 Re-run `evidence/probe_serial.py` against a real `Free` and
...
- [x] 8.1 Carry: a quaternion `Free`, and what a driver would bind for
  one.
- [x] 8.2 Carry: the MuJoCo `free` / Modelica `FreeMotion` export
  mapping. `Free` is the joint where both targets have a native element,
  so it is the cheapest one to get wrong by guessing.
- [x] 8.3 Carry: whether `at` is read in the parent's frame or the body's
  own. The hexapod cannot tell them apart; the rule chosen is the
  parent's frame, by consistency with ADR-088.
- [x] 8.4 Close: wiring a `Free` coordinate downward is REFUSED BY NAME
...
- [x] 8.5 Close or carry: the cost of re-placing six operations on every
