...
- [x] 1.1 Add `tests/mesh_engine_absent.py`: a helper that runs a callable in a
...
- [x] 1.2 Add `tests/test_mesh_engine_dependency.py::test_assertion_module_imports_without_the_mesh_engine`:
...
- [x] 1.3 Add `tests/test_mesh_engine_dependency.py::test_exact_assembly_asserts_without_the_mesh_engine`:
...
- [x] 1.4 Add `tests/test_mesh_engine_dependency.py::test_faceted_interference_names_the_missing_mesh_engine`:
...
- [x] 1.5 Add `tests/test_mesh_engine_dependency.py::test_gravity_support_names_the_missing_mesh_engine`:
...
- [x] 1.6 Add `tests/test_manifold_cache.py::test_no_manifold_is_built_for_an_exact_assembly`:
...
- [x] 1.7 Add `tests/test_manifold_cache.py::test_a_mixed_assembly_builds_one_manifold_per_faceted_comparison`:
...
- [x] 1.8 Record every red transcript in the change record before implementing.

## 2. Introduce the conditional mesh-engine dependency

- [x] 2.1 Add `solid_node/mesh_engine.py`, mirroring `solid_node/openscad.py`:
      ...
      handles.
- [x] 2.2 Remove `from manifold3d import Manifold, Mesh` from
...
- [x] 2.3 Run 1.2 green. Confirm the error text of 1.4/1.5 names the dependency,
      the operation and the reason.

## 3. Build the Manifold only where a faceted comparison reads it

- [x] 3.1 Split the per-STL cache: add `_cached_local_bounds(stl_file)` doing the
...
- [x] 3.2 Change `_solid_geometry` to read bounds from `_cached_local_bounds` and
      to carry what a later Manifold build needs, instead of building one.
- [x] 3.3 Change `_place_solid` to record a deferred placed Manifold in slot 1,
      preserving the four-slot record shape every consumer unpacks.
- [x] 3.4 Force the deferred placed Manifold at exactly its four consumers: the
...
- [x] 3.5 Confirm the bounds source is unchanged for every solid, exact or
      faceted, so no candidate set, grounded seed or virtual floor moves.
- [x] 3.6 Run 1.3, 1.6 and 1.7 green.

## 4. Prove nothing else moved

- [x] 4.1 Run `tests/test_manifold_cache.py` in full and confirm
...
- [x] 4.2 Run `tests/test_assembly_integrity.py`, `tests/test_assembly_supported.py`,
      `tests/test_broad_phase_culling.py`, `tests/test_assertions.py`,
      `tests/test_connectivity.py`, `tests/test_exact_geometry.py`.
- [x] 4.3 Run `tests/test_meta.py` in full: every green fixture green, every
...
- [x] 4.4 Re-run the originating reproduction end to end — an all-exact assembly
      under a genuinely absent `manifold3d` — and record the transcript beside
      the base transcript in the change record.
- [x] 4.5 Run the full suite and compare against this cycle's recorded base
...
- [x] 5.1 Document the conditional mesh-engine dependency where the OpenSCAD one
      is documented, naming the exact requiring paths and the all-exact
      guarantee.
- [x] 5.2 Assess ADR need after implementation. The candidate is a
