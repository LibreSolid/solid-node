...
- [x] 1.1 Extend `tests/step_project/` in the shape it already has — every
      ...
      that is a pure translation; a placement that is a 180° turn. NOT
      authored, and found not to be authorable through this toolchain
      (see the finding recorded at 4.1 below): a product placed through
      a mirrored or scaled occurrence transform. The propriety gate
      is instead proved directly on constructed `gp_Trsf`s and by
      forcing the gate's own function to report one real occurrence
      improper (`tests/test_step_assembly.py::PropertyGateUnitTest`).
- [x] 1.2 A fixture whose product names carry the shapes design fact 10
...
- [x] 2.1 Red: `tests/test_step_assembly.py` — `StepAssembly(path)` reports
...
- [x] 2.2 Red: `occurrences` reports one entry per placement, walked
      ...
      yields a distinguishable occurrence. FINDING: the spec's "reported
      label name is empty" does not hold on `cadquery.Assembly`'s own
      output — its component labels carry its own placeholder names
      ('2', or the product's own name at the root level), never a blank
      string; the test asserts the normative property that matters
      (identity is never derived from the label name) and records the
      discrepancy in its docstring rather than papering over it. A file
      holding one part and no assembly yields one occurrence at the
      identity.
- [x] 2.3 Red: an occurrence carrying no colour of its own reports none
      even when its product is coloured (design fact 9).
- [x] 2.4 Implement: the `StepAssembly` section of
...
- [x] 3.1 Red: for every occurrence of every fixture, composing
      ...
      translations.
- [x] 3.2 Red: the same round trip over all **55** occurrences of
      ...
      fails here rather than in a project. Confirmed passing (worst error
      asserted < 1e-12) against the actuator file present in this
      workspace at `projects/Internal-Cycloidal-Actuator/...`.
- [x] 3.3 Red: a cross-check of the reader's decomposition against an
      ...
      sign).
- [x] 3.4 Implement: decomposition through `gp_Trsf.GetRotation()` and
...
- [x] 4.1 Red: a mirrored placement is reported improper with its
      ...
      FINDING (evidence against design/spec, reported per the apply
      contract rather than worked around silently): no mirrored or
      scaled OCCURRENCE placement is authorable through this toolchain.
      Confirmed on two independent routes in this session --
      `cadquery.Assembly.add(shape, loc=cq.Location(gp_Trsf))` with
      `SetMirror`/`SetScale`, and raw `XCAFDoc_ShapeTool.AddComponent`
      with a `TopLoc_Location` built the same way -- writing through
      `STEPCAFControl_Writer` and reading back with
      `STEPCAFControl_Reader` in both cases resets the component's
      transform to the identity. This is not merely an OCCT writer gap:
      STEP's `NEXT_ASSEMBLY_USAGE_OCCURRENCE` occurrence mechanism
      places a component via the relative transform between two
      `AXIS2_PLACEMENT_3D` frames, each of which is right-handed BY
      CONSTRUCTION (its second axis is the cross product of the two it
      states), so the relative transform between any two such frames is
      always proper -- a mirror or scale cannot be expressed through
      this mechanism at all, only through a different one
      (`CARTESIAN_TRANSFORMATION_OPERATOR_3D`/`MAPPED_ITEM`) that this
      toolchain's writer does not use for assembly occurrences. The
      gate itself is still real and worth keeping -- a document written
      by a different tool using that other mechanism could carry one --
      so it is proved directly instead: on constructed `gp_Trsf`s
      (`PropertyGateUnitTest`), and by forcing the gate's own function
      to report one real occurrence of an all-proper fixture improper,
      confirming the walk still reports the other three and does not
      raise (`PropertyGateUnitTest.test_an_improper_occurrence_is_not_
      decomposed_by_the_walk`). The same substitution is used for
      `tests/test_import_step.py`'s command-level refusal test, since
      no improper STEP file can be handed to it either.
- [x] 4.2 Red: none of the actuator document's 55 occurrences is reported
      improper (skipped when the file is absent) — design fact 4.
      Confirmed passing against the actuator file present in this
      workspace.
- [x] 4.3 Implement: the gate on `|det(R) − 1| > 1e-9` or
      `|gp_Trsf.ScaleFactor() − 1| > 1e-9`, reported on the occurrence.
      Turn 4.1–4.2 green.

## 5. The generated source (red first)

- [x] 5.1 Red: `tests/test_import_step.py` — the class-name rule on the
      ...
      document (design D7).
- [x] 5.2 Red: `parts.py` holds one `StepNode` subclass per part and none
      ...
      colour.
- [x] 5.3 Red: `assembly.py` holds one `AssemblyNode` subclass per
...
- [x] 5.4 Red: each class's `render()` places every child by
...
- [x] 5.5 Red: the generated modules import cleanly, the root class
      ...
      two-level nest.
- [x] 5.6 Implement: `solid_node/manager/import_step.py`'s source
...
- [x] 6.1 Red: `solid -h` lists `import-step` with its docstring; the
...
- [x] 6.2 Red: `--into` defaults to the current directory, creates the
...
- [x] 6.3 Red: a second run into a directory already holding `parts.py`
      writes neither file, names `parts.py`, and exits 1; the same for
      `assembly.py` alone (design D9 — both files or neither).
- [x] 6.4 Red: a document with a mirrored or scaled placement writes
      nothing, names the occurrence with its determinant and scale factor,
      and exits 1.
- [x] 6.5 Red: `pyproject.toml` is unchanged on disk and the manifest
      ...
      raise `ImportError`).
- [x] 6.6 Implement: the `Import Step` command class with
...
- [x] 7.1 Scaffold the actuator document into a scratchpad directory —
      ...
      MEASURED (scratchpad `actuator_proof/`, never inside the
      actuator's repository): `solid import-step` scaffolds
      `Part10010Stator`, `M4_12mmScrew`, `M3_35mmScrew` etc. exactly by
      the stated rule; the model name could not be `actuator` because
      `--into actuator` also names a root-directory `actuator/`, which
      `solid`'s own several-models rule refuses as a model name (a
      real, mildly instructive naming trap for a pilot following the
      printed manifest literally) — built as `internal_cycloidal_
      actuator` instead. `solid build` succeeds (no `errors.json`,
      `viewer.json` written) in **21.5 s** cold. Artifacts: **20**
      distinct STL/BREP pairs (one per distinct part product; the
      other 35 of the 55 occurrences are repeated placements of these
      sharing the same `uniq_id` and so the same artifact, exactly as
      design D8 predicts), 124 files total under `_build/`. STL total:
      **12,305,380 bytes (11.74 MiB)** — against the 19.91 MB the
      framework's inherited 0.1 rad default would cost for
      `Output_Shaft` ALONE (cycle B's own figure); this build's
      `Output_Shaft` STL is **1,762,084 bytes (1.76 MB)**, matching
      cycle B's `angular_deflection = 0.5` figure exactly.
- [x] 7.2 Compare, for each of the 55 occurrences, the built model's leaf
      ...
      MEASURED: all 55 generated leaves matched one-to-one against the
      reader's 55 occurrences (by `part` then by document order, so a
      repeated product's copies are matched independently); worst
      placement deviation over all 55: **4.93e-10 mm** — about twenty
      million times inside the 0.01 mm the spec asks, on the
      translation component of the composed world matrix (comparison
      done via `solid_node.node.base._compose_world_matrix`, which
      needs no meshing or bounding box, so this is the exact placement
      rather than a sampled point).
- [x] 7.3 Record the measured costs for the docs and the ADR: the cached
      ...
      MEASURED: document read+transfer (cold cache), **11.10 s**;
      `StepAssembly` structure extraction on the warm cache (21
      products' names/kinds/occurrence-counts/solid-counts/colours,
      the walk of all 55 occurrences with matrices and decompositions),
      **0.042 s** — same order of magnitude as design fact 7's 0.02 s,
      the small excess plausibly this run's own solid-count reads or
      machine variance; still roughly 260x cheaper than the read it
      follows.

## 8. Documentation and records

- [x] 8.1 `docs/cli.rst`: an `import-step` section — the grammar, the two
      ...
      writes in `simulate()`.
- [x] 8.2 `docs/leaf-nodes.rst`: cross-reference `import-step` from the
...
- [x] 8.3 `docs/changelog.rst` "Unreleased": an entry in the style of the
...
- [x] 8.4 ADR in `docs/adrs/NODE/` — reading a STEP document's placements
...
- [x] 9.3 Report the follow-ups to the pilot: the shop's
      ...
      MEASURED (9.1): after the host reboot, `PYTHONPATH=$PWD .venv/bin/python -m pytest tests -q`: 1438 passed, 14 skipped, 234 subtests passed in 212.67 s; the 38 warnings are the pre-existing build123d deprecations.
