...
- [x] 1.1 Add `tests/test_build_publication.py` with a test that polls the
      published viewer file from a reader thread across repeated
      publications and asserts it is never absent.
- [x] 1.2 Run it and observe it fail on the current two-rename publisher,
      recording the observed absence count.

## 2. Red: overlapping publications

- [x] 2.1 Add a test that runs concurrent publishers against one build path
      and asserts no publisher raises and the final tree comes wholly from
      one publisher.
- [x] 2.2 Add a test that a publication losing the race is reported as a
      build failure through `errors.json` rather than escaping
      `Builder._publish()`.
- [x] 2.3 Add a test that a publisher removing its superseded artifact set
      does not remove a concurrent publisher's fresh one.
- [x] 2.4 Run all three and observe them fail.

## 3. Red: Git invisibility

- [x] 3.1 Add a test that a project whose ignore file does not cover the
...
- [x] 3.2 Add a test that a project whose ignore file already covers them
      gets no local exclusion written.
- [x] 3.3 Add a test that a missing or unwritable git directory is ignored
      and publication still succeeds.
- [x] 3.4 Run them and observe them fail.

## 4. Implement publication

- [x] 4.1 Rewrite `BuildSessionPublisher.publish()`: rename the completed
...
- [x] 4.2 Migrate a build path that is still a plain directory: move it aside
      once, install the symlink, then remove the moved directory.
- [x] 4.3 Remove only the versioned directory this publication superseded;
      never sweep siblings.

## 5. Implement Git invisibility

- [x] 5.1 Add a helper that records `<build-basename>*` in
...
- [x] 5.2 Call it from publication, idempotently.
- [x] 5.3 Update `solid_node/manager/templates/project/gitignore` so new
      projects cover the build path and its versioned directories.

## 6. Implement the failure channel

- [x] 6.1 Guard the `Builder._publish()` call site so a publication error
      becomes a reported build failure via the existing error path instead of
      an escaping exception.
- [x] 6.2 Confirm a failed publication leaves the other publisher's complete
      tree untouched.

## 7. Green and regression

- [x] 7.1 Run the new publication and Git-invisibility tests green, including
      the reader-gap test that previously counted absences.
- [x] 7.2 Run `tests/test_builder_lifecycle.py`, `tests/test_manager_build.py`,
      `tests/test_manager_develop.py` and `tests/test_export.py` green.
- [x] 7.3 Run the full framework suite and report the result.
- [x] 7.4 Verify by hand in this worktree that a real `solid build` publishes
...
- [x] 7.5 Verify by hand that a project built under the old layout migrates on
      its first publication.

## 8. Records

- [x] 8.1 Write an ADR under `docs/adrs/BUILD/` recording the symlink-swap
...
- [x] 8.2 Update `docs/architecture.md` if the publication description there
      no longer matches.
- [x] 8.3 Note the build-path layout change in `HISTORY.rst`, including that
      a pre-existing project cloned elsewhere may need one ignore line.
- [x] 8.4 Sync the `build-pipeline` and `one-shot-build-and-notification`
