...
- [x] 1.1 Extend lock-participant tests to assert that builder assembly and the
...
- [x] 1.2 Add a process-level cold CadQuery build regression and capture the
  current STL publication while an independent holder owns the project lock.
- [x] 1.3 Add a process-level cold `StlNode` test-build regression and capture
  imported mesh materialization before the independent holder releases the lock.

## 2. Critical-section correction

- [x] 2.1 Move builder assembly into the existing uninterrupted project-locked
  region, with before/after source checks and precise watcher startup.
- [x] 2.2 Defer builder assembly error handling until after lock release so a
  develop reload can wait without blocking other producers.
- [x] 2.3 Lock the test runner once across keyframing, preliminary render,
  assembly, and `build_stls()`, releasing before project test cases execute.
- [x] 2.4 Run focused lock, builder-lifecycle, test-manager, exact-adapter, and
...
- [x] 3.3 Synchronize and archive `lock-artifact-assembly`; record that no ADR
