...
- [x] 1.1 Create `tests/test_stl_node.py` with a module docstring
...
- [x] 1.2 Write failing tests for source declaration and freshness:
...
- [x] 1.3 Write failing tests for artifact materialization: round-trip
  ...
  does for JScad).
- [x] 1.4 Write failing tests for the watertight gate: non-watertight
  ...
  selected body.
- [x] 1.5 Write failing tests for multi-body packs: a three-body
...
- [x] 1.6 Write failing tests for `adjust`: a scaling hook reaches the
  artifact (volume reflects it); a node without the hook imports
  verbatim.
- [x] 1.7 Write failing adapter-contract tests: `exact` is False and
...
- [x] 1.8 Run the new test module and record the red evidence (every
  test fails for want of `StlNode`, not for fixture bugs).

## 2. Implementation

- [x] 2.1 Implement `solid_node/node/adapters/stl.py`: `StlNode(LeafNode)`
...
- [x] 2.2 Implement materialization in `as_scad()` (D3): up-to-date
...
- [x] 2.3 Export `StlNode` from `solid_node/node/__init__.py`.
- [x] 2.4 Run the full framework test suite; make the new module green
  without disturbing existing tests.

## 3. Representative caller

- [x] 3.1 Add a meta-project (or equivalent end-to-end) fixture
...
- [x] 3.2 Verify the fusion path end to end once: a `FusionNode`
...
- [x] 4.1 Add the STL leaf kind to `docs/leaf-nodes.rst`: declaration,
  ...
  doctrine.
- [x] 4.2 Update the adapter roster in `docs/architecture.md`.
- [x] 4.3 Extract ADR(s) for the accepted architectural choices —
...
- [x] 5.1 Sync delta specs to the baseline specs and archive the
  change (OpenSpec sync + archive workflows).
- [x] 5.2 Run final full test suite and `openspec validate`; confirm
