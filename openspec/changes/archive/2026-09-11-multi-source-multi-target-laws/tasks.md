...
- [x] 0.2 **Confirm the stack.** Cycle 1 (`repeat-fan-out`, ADR-096) must
...
- [x] 0.3 Record the FULL SUITE at the base:
...
- [x] 0.4 Re-run the two proposal probes and paste their current output
...
- [x] 0.5 Re-read `design.md` §14 and confirm with the orchestrating
...
- [x] 1.1 **Fixtures.** `Rod`, a rigid leaf declaring four joints
...
- [x] 1.2 **A group of sources drives one coordinate.**
...
- [x] 1.3 **A tuple of driven ends.** `a.drives((b, c), law=...)` and
  `a.drives(b & c, law=...)` state the same relation. RED (today:
  `(...) is not a coordinate`).
- [x] 1.4 **`&` chains flat.** `x & y & z` is one group of three, in
  written order, not a nested pair — assert the ends the relation
  carries, not the object.
- [x] 1.5 **Every group refusal, each by its own message** (design §7):
...
- [x] 1.6 **The existing per-member refusals are reached inside a group**
...
- [x] 2.1 **What the law is handed.** A relation of three sources and
  ...
  `simulate()`.
- [x] 2.2 **A one-to-one law is untouched.** The same two-argument
...
- [x] 2.3 **Mixed arities.** n sources with one driven end hands a tuple
  and a node; one source with m driven ends hands a node and a tuple.
- [x] 2.4 **`forward` spreads its sources and returns a sequence.** Three
  ...
  all accepted.
- [x] 2.5 **A wrong return shape is refused by name**, at the moment it
...
- [x] 2.6 **A single driven end still returns a bare value**, including
  when the value is symbolic; nothing is unpacked.
- [x] 2.7 **No new error kind.** The shape refusal is a `CouplingError`
  and the three named errors are still exactly three — assert the module
  exports.

## 3. Red first: the solver

- [x] 3.1 **Forward once, when the last source is bound.** Two sources
...
- [x] 3.2 **Deferred across the tree.** The last source is bound by a
  ...
  next run.
- [x] 3.3 **Never inverted.** All m driven ends bound by hand and the
...
- [x] 3.4 **The unreached message names the unbound sources.** Two of
  three sources bound, nothing reaches the third:
  `UnreachedCoordinate` naming the ONE unbound source, not all three.
- [x] 3.5 **Claim before bind.** The author binds one of four driven ends
...
- [x] 3.6 **The second run re-solves**: three successive instants, each
  clearing what the last bound, every driven end holding the current
  instant's value, nothing refused as doubly bound.
- [x] 3.7 **Symbolic values pass through.** With nothing bound, each of
  ...
  strings, one per end.
- [x] 3.8 **Order is unchanged.** A class declaring relation A, then a
...
- [x] 4.1 **Six copies, four ends, six law calls.**
...
- [x] 4.2 **The named relation reads as six records in copy order**, each
  with `direction == 'forward'`, and each `described()` naming the ends
  as written and the copy.
- [x] 4.3 **A doubly bound copy is refused naming that copy**, and the
  other copies' records are unaffected by the message's content.
- [x] 4.4 **Several sources, one driven end, over a repeat**:
  `(x & y & z).drives(towers.height, law=...)` over `.repeat(3)`.
- [x] 4.5 **A zero-count repeat with several driven ends is zero
  records** — binds nothing, refuses nothing.
- [x] 4.6 **`copy` is the node the repeated segment realized.** With a
...
- [x] 5.1 `solid_node/motion/couplings.py`: `Coordinates` (what `&`
...
- [x] 5.2 `RelationRecord` carries `driver_ends` and `driven_ends`
  TUPLES; `driver_end`/`driven_end` stay as properties for the
  one-to-one case; `described()` names several ends in written order.
- [x] 5.3 `Relation.resolve`: resolve the driver ends; resolve the driven
  ...
  segment.
- [x] 5.4 `_step_relation`, `_claim`, `_refuse`: all sources bound →
  claim all, apply, check the shape, bind all; any source unbound →
  defer; never backwards. The messages of design §6.
- [x] 5.5 The `&` operator beside `drives`, on exactly the declarations
...
- [x] 5.7 `tests/coupling_project/`: one tree that states a two-source,
...
- [x] 6.2 **Pose comparison over every project on the motion layer, base
...
- [x] 6.3 Cost: `pytest tests/test_couplings.py tests/test_kinematics.py`
...
- [x] 7.1 **kossel: all four of a rod's joints from one law, under
...
- [x] 7.2 **The Pascaline's pawl.** The overlay states
...
- [x] 7.3 **OpenFlexure's four legs.** The overlay declares `lean` and
  ...
  (design §15).
- [x] 7.4 **InMoov's hand.** The overlay states
...
- [x] 7.5 Confirm with `git status` that nothing under `projects/` was
  written, and say so in the report.

## 8. Specs, decision record and docs

- [x] 8.1 **Re-base the delta.** Two of the four MODIFIED requirements
...
- [x] 8.2 If cycle 4's ADDED requirement "Relations defer to a whole-tree
  ...
  uses.
- [x] 8.3 A new ADR under `docs/adrs/NODE/` at the next free number: *a
...
- [x] 8.4 `docs/architecture.md` §Couplings: several ends at one
  relation, in the reference voice — rewrite the paragraph that says an
  end is one coordinate rather than appending to it.
- [x] 8.5 `docs/driving.rst`: a passage for the several-ended relation
...
- [x] 8.6 `docs/api-reference.rst` and `docs/changelog.rst` Unreleased.
  `tests/test_docs_exports.py` passes unchanged — confirm rather than
  assume.

## 9. The findings

- [x] 9.1 `workflow/warts.md`: mark FIXED the multi-source finding in its
...
- [x] 9.2 `workflow/docs/motion-catalogue-2.md` §3.5: it is the
  ...
  sighting.
- [x] 9.3 Do NOT touch `libresolid-studio/docs/motion-general-refactor.md`
...
- [x] 10.1 `openspec validate --strict multi-source-multi-target-laws`.
- [x] 10.2 Sync the delta specs into the baselines and archive the
  change, only after the orchestrating session's review.
- [x] 10.3 Report: the suite counts, the pose comparisons, the four
...
Four projects (BCN3D-Moveo, openarm, openvmp, Internal-Cycloidal-Actuator)
could not be captured from a `git archive` snapshot because their
vendored meshes/STEP are untracked; they are re-measured from their own
clean working trees, base (main 9914a2e) against head, after this cycle
is rebased onto main, and the numbers are appended to evidence.md. The
delta's "Relations defer to a whole-tree fixpoint" block was refreshed
with the six scenarios cycle 4's archive added to the baseline after
this delta was written.
