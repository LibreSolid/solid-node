...
- [x] 1.2 Red: after a full build, editing one leaf's class body re-derives
      that leaf and the fusion and restamps the other leaf without rendering
      it.
- [x] 1.3 Guard: editing a module-level constant or helper in `parts.py`
      re-derives every node in it.
- [x] 1.4 Guard: a leaf that reaches its sibling — as a base class, through a
      module-level helper that names it, and by its name as a string — is
      re-derived when the sibling's body is edited.
- [x] 1.5 Guard: a single-class file's digest entry equals the sha256 of its
      bytes, so `source_digest` for the existing currency fixture is unchanged
      byte for byte and its recorded sidecars still match.
- [x] 1.6 Guard: the fast path reads no source — the existing test stays green
      unchanged.
- [x] 1.7 Guard: a `parts.py` whose sidecars carry whole-file digests
      rebuilds its nodes once after being touched, then reports current.

## 2. The scope beside the file set

- [x] 2.1 A node records `scope = {realpath(src): {ClassName}}` for a Python
      source when it is constructed, beside `self.files`.
- [x] 2.2 `InternalNode.as_scad` unions each child's scope where it unions
      `child.files`.
- [x] 2.3 `node.source_digest` passes the scope to `currency.source_digest`.

## 3. Retained text

- [x] 3.1 Extend the loaded-module index in `node/sources.py` (ADR-058) to
...
- [x] 3.2 One cached analysis per `(path, mtime_ns, size)`: the top-level
...
- [x] 3.3 `retained_text(path, keep)`: remove the spans of node classes not in
...
- [x] 3.4 `currency.source_digest(files, root, scope)` digests a scoped file's
...
- [x] 4.1 Turn every red and guard test from group 1 green.
- [x] 4.2 Full framework suite.
- [x] 4.3 On a real project laid out one node per file (v8-engine), confirm a
...
- [x] 4.4 Confirm `solid develop` still watches the same file set.

## 5. Records

- [x] 5.1 ADR-071 (numbered 069 when planned; 069 and 070 were taken by parallel cycles) in `docs/adrs/NODE/`: node-scoped content currency; amends
...
- [x] 5.2 Update `docs/adrs/README.md` and the node-model summary in
      `docs/architecture.md`.
- [x] 5.3 Changelog entry; sync the `build-pipeline` spec; archive the change.
