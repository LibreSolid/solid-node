...
- [x] 1.1 Add focused all-model regressions for construction/load, render,
  assembly, and artifact-generation failures, requiring each to count once.
- [x] 1.2 Prove a non-failfast run executes a later model's tests, prints one
  aggregate report, and exits 1 after an earlier model fails to prepare.
- [x] 1.3 Prove `--failfast` records the first preparation failure, skips the
  later model, still prints the aggregate report, and exits 1.
- [x] 1.4 Run the saved real CLI probe before and after the fix, requiring
  `second_ran: true` after implementation while preserving exit status 1.

## 2. Model-scoped continuation

- [x] 2.1 Introduce one model-failure accounting helper shared by unresolved
  references and failures during model preparation.
- [x] 2.2 Place the all-model preparation boundary around load/construction,
  keyframe binding, render, assembly, and artifact generation without
  catching interpreter-control exceptions.
- [x] 2.3 Continue with a clean next selection without `--failfast`; otherwise
  raise the existing `StopTestRun` signal after accounting.
- [x] 2.4 Preserve single-model diagnostics/control flow and ordinary
  test-method accounting.

## 3. Completion

- [x] 3.1 Run focused manager, named-model, CLI, loader, and build coverage,
  then the saved probe.
- [x] 3.2 Run the complete framework suite and strict OpenSpec validation.
- [x] 3.3 Update the changelog and due-diligence records, mark F09 complete in
  `PROGRESS.md`, and identify F10 as next.
- [x] 3.4 Synchronize and archive
...
- [x] 3.5 Commit the completed implementation as the second F09 commit, then
