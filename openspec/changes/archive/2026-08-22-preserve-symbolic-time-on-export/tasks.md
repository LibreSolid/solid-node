...
- [x] 1.1 In `tests/test_export.py::ExportAnimationTest`, add
...
- [x] 1.2 In `tests/test_export.py::ExportAnimationTest`, add
...
- [x] 1.3 In `tests/test_keyframe_reversal.py` (new module; `tests/test_meta.py`
      turned out to be a subprocess `solid test` harness, the wrong layer for
      these — this sits beside `tests/test_conrod_symbolic.py` instead), add a
      keyframe-reversibility test over
      ...
      'clear_keyframe'`).
- [x] 1.4 In `tests/test_meta.py`, add the non-linear counterpart over
...
- [x] 1.5 In `tests/test_meta.py`, add the accumulation guard: alternate
      `set_keyframe`/`clear_keyframe` twice and assert the driven child holds
      exactly one rotation operation. Record it red.
- [x] 1.6 In `tests/test_meta.py` (or beside the existing assembly tests), add
...
- [x] 2.1 Add `AbstractBaseNode.clear_keyframe()` in `solid_node/node/base.py`
      as a documented no-op, beside the existing `set_keyframe` no-op.
- [x] 2.2 Add `AssemblyNode.clear_keyframe()` in `solid_node/node/assembly.py`:
...
- [x] 2.3 Add the shared non-list/tuple guard to both `set_keyframe` and
      `clear_keyframe` so a partial node recurses into no children instead of
      raising, matching `serialize_node`'s tolerance.
- [x] 2.4 Run 1.3–1.6 green.

## 3. Make export guarantee the animated document

- [x] 3.1 In `solid_node/core/export.py`, have `export_node` call
      `node.clear_keyframe()` before `serialize_node`.
- [x] 3.2 Update `export_node`'s docstring and the module docstring: the
...
- [x] 3.3 Run 1.1–1.2 green.

## 4. Prove nothing else moved

- [x] 4.1 Run `tests/test_export.py`, `tests/test_pieces.py`,
