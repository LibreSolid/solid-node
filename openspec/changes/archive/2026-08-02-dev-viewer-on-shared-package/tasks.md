...
- [x] 1.1 Add a browser test that builds the `spinner_project` fixture, serves
...
- [x] 1.2 Add failing tests for the new routes: `GET /build/viewer.json`
...
- [x] 4.2 Reduce `App.tsx` to that shell plus the error pane, and set the tab
      title from the snapshot root's name.
- [x] 4.3 Point the reloader's reload callback at the mount handle's `reload()`
...
- [x] 4.4 Show the reported remedy in the error pane when the bundle is
      unavailable, with a jest test for that path.
- [x] 4.5 Delete `node.ts`, `evaluator.ts`, `animator.ts`, `operations.d.ts`,
      ...
      `evaluator.test.ts`).
- [x] 4.6 Remove the CSS rules that belonged to the deleted panes, keeping the
      offline banner and error styles.

## 5. Evidence

- [x] 5.1 Run the framework suite; the browser test from 1.1 now passes with
      colours, lights and a fitted camera.
- [x] 5.2 Run the app's jest suite and its production build; run the widget
      package's `npm test`, `npm run typecheck` and `npm run build`
      unchanged.
- [x] 5.3 Open `solid develop` on a real project from this bench, confirm the
      ...
      build-error pane.
- [x] 5.4 Confirm `--web-dev` still serves the page through the npm dev server
      with the snapshot and bundle routes coming from the backend.

## 6. Dependencies, records and documentation

- [x] 6.1 Remove `three`, `@types/three`, `jokenizer`, `re-resizable`,
...
- [x] 6.2 Update `docs/architecture.md`'s web-viewer synthesis to describe a
      snapshot-served shell around the shared viewer.
- [x] 6.3 Extract the ADRs the implementation confirms (supersede ADR-014,
      supersede or narrow ADR-027, amend ADR-013) and update the ADR index.
- [x] 6.4 Synchronize the `web-viewer` and `build-viewer-artifacts` specs,
