...
- [x] 1.1 Create `tests/test_mechanisms.py` importing every flat name from
  `solid_node.mechanisms`; prove it red on the missing package.
- [x] 1.2 Gear tests: the reference-pose identity (`driver_angle = line -
...
- [x] 1.3 Screw tests: `screw_travel(360, 2.0) == 2.0`, `screw_angle(1.0,
  2.0) == 180`, and the inverse identity over a sweep.
- [x] 1.4 Crank tests: the dead-centre values for radius 15 and rod 60, and
...
- [x] 1.5 Delta tests: the three effector positions from the spec for rod
  ...
  and tower angles.
- [x] 1.6 Linkage tests: `circle_intersection((0, 0), 5, (8, 0), 5, ±1)`,
  ...
  `link_rise(5, 3) == 4`.
- [x] 1.7 Symbolic-face tests: every exported law called with a symbolic
...
- [x] 1.8 Agreement test: for every law, evaluate the symbolic string at
  ...
  face.
- [x] 1.9 Declared-face refusal test: a class body computing
...
- [x] 2.1 Create `solid_node/mechanisms/__init__.py` with a package
...
- [x] 2.2 `gears.py`: `meshed_angle` and `driving_angle`, module docstring
...
- [x] 2.8 Run `tests/test_mechanisms.py` green, then the full framework
  suite.
...
- [x] 3.2 wall_clock_01: assert `pinion_angle_for_wheel` and
...
- [x] 5.2 Add a "Mechanisms" subsection and a map row to
  `docs/architecture.md`.
- [x] 5.3 Sync the baseline specs from the delta and archive the change.
- [x] 5.4 Report to the pilot the two open questions in `design.md`.
