...
- [x] 1.1 Add `tests/test_packaging.py` driving the `sdist` and `build_py`
...
- [x] 1.2 Extend that test to assert the widget's built bundle is selected by
      the distribution's file rules when present in the tree.
- [x] 1.3 Add `tests/test_manager_viewer.py` asserting `solid viewer` reports
      ...
      no such command.
- [x] 1.4 Add a test asserting `solid -h` lists `viewer` and that invoking it
      requires no node path.

## 2. The accessor

- [x] 2.1 Add `solid_node/viewers/bundle.py` with standard-library imports
...
- [x] 2.2 Add its unit tests, including that the reported API version equals
      `solidNodeViewerApi` and that importing the module pulls in no CAD
      runtime.
- [x] 2.3 Point `solid_node/core/export.py` at the accessor, keeping
      `WidgetBundleMissing` and its message behavior.
- [x] 2.4 Point `solid_node/sphinx.py` at the accessor and confirm the
      documentation extension still imports without the CAD stack.

## 3. The command

- [x] 3.1 Add `solid_node/manager/viewer.py` with `needs_node = False`,
      printing one JSON object and exiting 1 with the remedy when absent.
- [x] 3.2 Register it in `solid_node/cli.py`.

## 4. Packaging

- [x] 4.1 Teach `solid_node/packaging.py` a second frontend directory: build
      the widget during sdist, and during wheel build when its bundle is
      missing.
- [x] 4.2 Update `MANIFEST.in` so the built bundle is included and its comment
      states that packaging builds it.

## 5. Evidence and completion

- [x] 5.1 Run the framework suite; run the widget's `npm test`, `typecheck` and
      `build` if any package file changed.
- [x] 5.2 Run a documentation build with `-W` and the export and Sphinx tests.
- [x] 5.4 Extract an ADR if the accessor or the CLI report proves to be a
      consequential architectural decision; update `docs/architecture.md` if
      its synthesis changed.
- [x] 5.5 Synchronize specs, archive the change, and create the completion
      commit.
