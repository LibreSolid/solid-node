...
- [x] 1.1 Add the named boundary table to `tests/test_broad_phase_culling.py`:
...
- [x] 1.2 Add the exhaustive lattice: unit boxes at half-integer offsets over a
...
- [x] 1.3 Add a conservativeness test for `_world_bounds` under rotation: the
      bound encloses the placed geometry and is a strict superset of the
      untransformed local box
- [x] 1.4 Add a differential check over the assemblies the existing tests
      already construct, asserting candidate-set containment with no new
      geometry
- [x] 1.5 Confirm red twice: the table and lattice fail against a temporarily
...
- [x] 2.1 Rewrite `test_global_deficit_stays_positive_for_triple_overlap` and
...
- [x] 2.2 Confirm those rewritten tests pass against the current two-path
      implementation before anything is removed, so the coverage is proven
      independent of the removal
- [x] 2.3 Add two deliberately-red meta fixtures beside the existing
...
- [x] 3.1 Delete `_assembly_volume_certificate` from `solid_node/test.py`
- [x] 3.2 Reduce `assertNoSolidInterference` to selection, world bounds,
...
- [x] 3.3 Delete the certificate-patching tests
...
- [x] 3.4 Remove or reword any docstring and comment referring to the
      certificate, the deficit, or the uncertainty bound

## 4. Validate

- [x] 4.1 Run `tests/test_assembly_integrity.py`,
...
- [x] 4.2 Run the meta fixtures `assembly_integrity_nested`,
...
- [x] 4.3 Run the full framework suite and confirm no unrelated regression
- [x] 4.4 Verify the scaffold acceptance still reports exactly two passing
      tests for a generated leaf project

## 5. Measure

- [x] 5.1 Reproduce the clean-pass timings under the methodology in
...
- [x] 5.2 Measure the first-offending-pair (failing) case before and after
- [x] 5.3 Record both, with the deprecated-sweep comparison for continuity,
      in `docs/performance-improvement.md` as bounded observations

## 6. Records

- [x] 6.1 Revise ADR-040 in place: retitle away from "certificate", rewrite
...
- [x] 6.2 Update the ADR-040 row in `docs/adrs/README.md` for the new title
- [x] 6.3 Update `docs/testing.rst` and any other user-facing text that
      describes the union comparison
- [x] 6.4 Confirm no new ADR is created and ADR-040 keeps its `Accepted`
