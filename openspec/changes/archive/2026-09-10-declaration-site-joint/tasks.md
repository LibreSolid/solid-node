...
- [x] 0.2 **Cycle 2 must be integrated AND archived first.** Confirm that
...
- [x] 0.4 **The reference poses are cycle 2's BEFORE captures, for every
...
- [x] 0.5 Where a base pose must be re-measured, capture one project at a time, from the project's
...
- [x] 0.6 Re-read `evidence/sightings.md`. It is the empirical basis of
...
- [x] 1.1 **Thor's elbow, stated by its parent.** The inverse of cycle
...
- [x] 1.2 **A site joint anchors on the PARENT's origin.** A parent
...
- [x] 1.3 **One declaration, opposed placements, opposite own-frame
...
- [x] 1.4 **A site joint's run sits at its own slot, inside the rest
...
- [x] 1.5 **A site joint of a name the class declares keeps that slot.**
  ...
  reads `pip`, `mcp`. RED.
- [x] 1.6 **A site joint of a new name comes last, in keyword order.** A
  class declaring `spin`, a site passing `orbit=` then `lift=`:
  enumeration `spin`, `orbit`, `lift`. RED.
- [x] 1.7 **A symbolic rest placement refuses a SITE joint and not a
  ...
  UNEDITED.
- [x] 1.8 **A site `Free` floats against the parent's frame.** A parent
...
- [x] 1.9 **A site `Prismatic`'s axis is carried and its anchor is
...
- [x] 2.1 **A defaulted site `carries` is the CHILD's own origin.** A
...
- [x] 2.2 **Six copies, one declaration, six derived phases.** Six copies
...
- [x] 2.3 **A written site `carries` is a point of the PARENT's frame.**
...
- [x] 2.4 **A site `Orbit` whose defaulted carried point lands on the
...
- [x] 2.5 **Cycle 2's class-declared `Orbit` cases pass UNEDITED**,
...
- [x] 3.1 **A site joint is not a parameter and not identity.** A
...
- [x] 3.2 **A wiring still means a wiring.** A keyword whose value is a
...
- [x] 3.3 **Every refusal of design decision 10**, one case each, each
...
- [x] 3.4 **A site joint on a `.repeat()` and on a literal list.** Every
  copy carries the freedom, all copies resolve identical arguments, and
  each is carried through its own placement. RED.
- [x] 3.5 **A callable is handed the declaring parent.** Two parents with
...
- [x] 3.6 **A copy's `index` is NOT reachable from a site callable.**
...
- [x] 4.1 **The descriptor protocol.** On a child whose `turn` came from
...
- [x] 4.2 **`declared_ports` and `declared_joints`** of the realized
...
- [x] 4.3 **`get_coordinate` / `set_coordinate`** answer for a site name,
...
- [x] 4.4 **A relation names a site coordinate by path**, driver and
...
- [x] 4.5 **A bare child end means the one joint the declaration gives
  it**, and a child with a class joint AND a different site joint is
  refused as a two-joint node by name.
- [x] 4.6 **A path naming a keyword no site passed is refused at class
  definition**, naming the path, the segment and what the class declares.
- [x] 4.7 **`capture_poses.py` sees a site joint.** Build a two-node
...
- [x] 5.1 **Identity is unchanged.** Two children of one class, declared
...
- [x] 5.2 **`isinstance` holds and the class reads as the written one.**
  ...
  file. RED.
- [x] 5.3 **One specialization per declaration site.** All six copies of
...
- [x] 5.4 **The specialization is invisible to model discovery.**
...
- [x] 5.7 **Class-keyed caches stay bounded.** After realizing a tree
...
- [x] 6.1 `solid_node/motion/joints.py`: a joint carries whether it was
...
- [x] 6.2 `solid_node/node/declarative.py`: split a coordinate-valued
...
- [x] 6.3 `solid_node/node/base.py`: `ChildDeclaration.realize` receives
  ...
  hook is added.
- [x] 6.4 Grep the framework for every reader of
...
- [x] 7.1 Every case of tasks 1-5 green.
- [x] 7.2 FULL SUITE: `.venv/bin/python -m pytest -x -q` from the
  worktree, exact counts in `evidence.md`, matched against 0.3. Any new
  failure stops the cycle.
- [x] 7.3 **No test of ADR-088, ADR-093, ADR-094, ADR-095 or ADR-096 was
...
- [x] 7.4 Assert the joints module's import cost is unchanged except for
...
- [x] 8.1 **The four projects cycle 2 leaves broken, against their
...
- [x] 8.2 **For the Internal Cycloidal Actuator, prove no forbidden
...
- [x] 8.3 **For OpenCycloid, prove the derived numbers stayed derived.**
...
- [x] 8.4 **The five axes and the four subclasses-for-metadata, against
...
- [x] 8.8 Record in `evidence.md`, per project: which base the reference
...
- [x] 9.1 `docs/driving.rst`, the joints section: the second half of the
...
- [x] 9.2 `docs/changelog.rst`, Unreleased: the entry, addressed to the
...
- [x] 9.3 The ADR: a new NODE ADR, "A joint may be declared where a child
...
- [x] 9.4 Update `docs/adrs/README.md` and `docs/architecture.md` where
  they describe where a joint is declared.
- [x] 9.5 `openspec validate --strict declaration-site-joint`;
  `openspec archive` when the pilot says so.
- [x] 9.6 File in `workflow/warts.md`:
