...
- [x] 1.1 In `tests/test_math.py`, add a symbolic-string test asserting the
...
- [x] 1.2 Add numeric-face tests for the six direct builtins over positive,
  negative, zero and exact-half inputs, pinning `floor(-2.5) == -3`,
  `ceil(-2.5) == -2` and `sign(0) == 0`.
- [x] 1.3 Add tests for each composition's stated behaviour: `clamp`,
...
- [x] 1.4 Extend the existing `_eval_openscad_expr` helper in
...
- [x] 1.5 Add a test that `min` (and one composition) called with a symbolic
  value and a declared parameter token raises, naming both operands, rather
  than embedding a `repr()` in the expression string.
- [x] 1.6 Add layer-2 tests: `polar`, `turn` with and without a centre, and
  ...
  component.
- [x] 1.7 In `tests/test_declarative_algebra.py`, add the dimension-rule
...
- [x] 2.1 In `solid_node/math.py`, bind the shadowed builtins privately at
...
- [x] 2.2 Add the mixed-face guard to the dispatch: a call carrying both an
  `OpenSCADConstant` and an `Expression` raises naming both operands.
- [x] 2.3 Implement `abs`, `floor`, `ceil`, `sign`, `min` and `max` with the
  module's three-face dispatch, `min` and `max` taking exactly two arguments.
- [x] 2.4 Implement `clamp`, `clamp01`, `ramp` and `lerp` as compositions.
- [x] 2.5 Implement `wrap(value, period=360.0)` as
  `value - period * ceil((value - period / 2) / period)`, validating a
  positive numeric period.
- [x] 2.6 Implement `piecewise(x, points)` as the sum of clamped ramps, with
  call-time validation of the waypoint sequence.
- [x] 2.7 Implement `bump(u)` as `p = clamp01(u); 16 * p * p * (1 - p) *
  (1 - p)` — a polynomial, not the trigonometric form, so the function has a
  declared face at all (design D5).
- [x] 2.8 Add the module-level `SYMBOLIC_BUILTINS` tuple naming every OpenSCAD
...
- [x] 2.9 Extend `__all__` and confirm `round` and `mod` are absent, with a
  module comment recording why (design D3, D4).

## 3. Layer 2: the vector helpers

- [x] 3.1 Implement `polar`, `turn(point, angle, about=(0.0, 0.0))`,
  `rotate_x`, `rotate_y` and `rotate_z` as composition over layer 1 only,
  returning plain tuples, and add them to `__all__`.
- [x] 3.2 Confirm that no name reaches `function_formula` except the six
...
- [x] 4.1 In `solid_node/parameters.py`, extend `function_formula` with a rule
...
- [x] 4.2 Verify that each composition's declared behaviour falls out of those
...
- [x] 4.3 Run `tests/test_declarative_algebra.py` and `tests/test_math.py`
  green.

## 5. The parity corpus

- [x] 5.1 Add `tests/expression_project/` — a small assembly declaring one
  ...
  also import.
- [x] 5.2 Extend `tools/generate_parity_fixture.py` to walk that tree the same
...
- [x] 5.3 Add a generator-side check that every name in
...
- [x] 5.4 Regenerate the fixture into `solid-node-viewer` and diff it: every
  ...
  condition.
- [x] 5.5 Run the viewer's vitest against the regenerated fixture and record
...
- [x] 6.1 Run the framework test suite and record the result.
- [x] 6.2 Confirm no existing expression string changed: run the tests that
...
- [x] 7.1 In `projects/abacus`'s own repository, replace `absolute`,
...
- [x] 7.2 Do the same for `projects/fender-bender`'s `piecewise` and `bump`,
  as the second caller covering the composition path.

## 8. Documentation

- [x] 8.1 Add an "Expression math" section to `docs/api-reference.rst`, which
  has none today, covering both layers.
- [x] 8.2 Extend `docs/animation.rst`'s "Non-linear kinematics" section with
...
- [x] 8.3 Extend `docs/declaring.rst`'s algebra section with the new dimension
  rules, including why `floor` and `ceil` want a dimensionless argument.
- [x] 8.4 Add the changelog entry under `Unreleased` in `docs/changelog.rst`,
...
- [x] 9.1 Append a revision note to
...
- [x] 9.2 Update the "Expression math" section of `docs/architecture.md` to
  the new state, rewriting rather than appending.
- [x] 9.3 Sync the baseline specs from the delta and archive the change.
- [x] 9.4 Report to the pilot: the `%` sign divergence found in D4 as a live
