...
- [x] 1.1 Write `tests/coupling_project/`, in the style of
...
- [x] 1.2 `tests/test_couplings.py`, the statement: `drives` on a child
...
- [x] 1.3 Ends: a port/joint of the class, a node standing for its one
...
- [x] 1.4 `Affine`: forward and inverse on numbers; the same on a
...
- [x] 1.5 The law protocol: a `law=` callable is called ONCE per
...
- [x] 1.6 Derived coordinates: `a - b`, `a + 2 * b`, `-a`, `a / 2`
...
- [x] 1.7 Freshness across runs: the three-arbor train enumerated at
...
- [x] 1.8 Wirings inside the fixpoint: an arbor whose coordinate a
...
- [x] 1.9 The solver: the three-arbor train binds only `escape.turn`
...
- [x] 1.10 The three refusals, each its own error kind exported from
...
- [x] 1.11 Symbolic and numeric faces: a tree of relations serialized
...
- [x] 1.12 Cross-cycle: a relation into a scaled port applies the scale
  ...
  port drives it.
- [x] 1.13 Import cost: `solid_node.motion.couplings` in a fresh
...
- [x] 2.3 `CoordinateRef` and its kinds — own port/joint, node, path,
  ...
  one rule.
- [x] 2.4 `DerivedCoordinate`: a coefficient mapping plus a constant,
...
- [x] 2.5 `Relation`: the two refs, the law or the ratio/offset pair,
...
- [x] 2.6 `UnreachedCoordinate`, `DoublyBound`, `NotInvertible`, and
...
- [x] 2.7 The solver: inventory with binder identity, the fixpoint over
...
- [x] 3.1 `declarative.py`: a mutable relations list created once in
...
- [x] 3.2 `declarative.py`: `ChildDeclaration.__getattr__` consults
...
- [x] 4.1 `assembly.py`: REWORK cycle 2's end-of-phase wiring binding
...
- [x] 4.2 The freshness clear: at the START of the phase, before the
...
- [x] 4.3 The per-enumeration binder record: what bound each coordinate
...
- [x] 5.1 `docs/architecture.md`: Kinematics gains the relation and
...
- [x] 5.2 `docs/declaring.rst`: the verb, path references, derived
...
- [x] 5.3 `docs/changelog.rst` "Unreleased": the verb, `Affine`, path
  references, derived coordinates, the sideways relaxation.
- [x] 5.4 `docs/adrs/NODE/ADR-089-drives-relates-two-coordinates.md`,
...
- [x] 6.1 The whole framework suite green, with the counts before and
  after. (Two complete runs during apply: 1921 passed / 3 failed -- the
  three stale `couplings is empty` expectations, then widened -- and
  1922 passed / 2 failed, both from two pytest processes sharing
  `tests/_build` and both green on rerun. A final confirming run on the
  finished tree did not complete: this machine slowed to a crawl in the
  subprocess-spawning and build-heavy sections, while the same files
  run in seconds on their own. It reached 59% with zero failures.)
- [x] 6.2 The fixture train's own evidence recorded in the change: the
...
- [x] 6.3 `openspec validate couplings --strict`, then sync the
  baseline specs and archive.
- [x] 6.4 `workflow/motion/roadmap.md` Progress table, row
