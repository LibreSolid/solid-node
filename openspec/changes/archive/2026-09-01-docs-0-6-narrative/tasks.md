...
- [x] 1.1 Write the two-axis printer demo (two `Axis` instances, one
      machine-level instruction), export it with `solid export`, and commit
      the widget-less export under `docs/_exports/`
- [x] 1.2 Write `docs/driving.rst`: driver declaration and attribute reads,
      ...
      committed export
- [x] 1.3 Write `docs/scenarios.rst`: `Sim`, `dt` and integer ticks,
      `at`/`every`/`trigger`/`run`, trajectories and stats, `ScenarioTest`
      under pytest and `solid test`
- [x] 1.4 Add both pages to the `docs/index.rst` toctree per design
      decision 1

## 2. Entry-surface rewrite

- [x] 2.1 Rewrite the `docs/index.rst` introduction around the machine
      narrative
- [x] 2.2 Rewrite `docs/why-solid-node.rst` (machine thesis, five backends,
      sheet/STL/flexible leaves, exactness, tests, Open Source)
- [x] 2.3 Rewrite `docs/quickstart.rst`: tiered requirements (conditional
      OpenSCAD), artifact inventory (`.stl`/`.brep`/`.dxf`), a first driver
      as the payoff, upgrade/reinstall note
- [x] 2.4 Update `README.rst` user-facing section: tagline, capability
      paragraph, `solid_node/simulation/` in the layout, reinstall note
- [x] 2.5 Update `docs/status-and-roadmap.rst`: 0.6 as released, upgrade
      warning, roadmap rebuilt from the changelog's deliberate deferrals
- [x] 2.6 Rewrite `docs/examples.rst` around both example projects: the V8
...
- [x] 3.6 `docs/viewer.rst`: "Driving the model" section linking
      `driving.rst`, OpenSCAD-path caveat for driven machines
- [x] 3.7 `docs/embedding.rst`: API version 5, schema versions 1/2/3 and
...
- [x] 5.1 Author the 0.6.0 entry in `docs/changelog.rst` from the release
      branch's `HISTORY.rst` text
- [x] 5.2 Write `docs/releases/release-0.6.md` on the `release-0.5.md`
      template
- [x] 5.3 Refresh `conf.py` `autodoc_mock_imports` for the 0.6 dependency
      set, preserving the bumpversion-managed `release` line

## 6. Verify the record

- [x] 6.1 Run a full `sphinx-build` treating warnings as errors, on
      committed exports only, and fix what it reports
- [x] 6.2 Review the built HTML against every scenario of the
