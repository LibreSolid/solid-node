...
The system SHALL expose from `solid_node.node` exactly the names it exposes
today, each resolved when it is first accessed rather than when the package is
imported. Importing `solid_node.node`, or any module beneath it, SHALL NOT
import a backend the caller has not named.
