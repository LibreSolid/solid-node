...
package name, so the existing companion-file mapping discovers the test
without a new loader convention. The generated test SHALL be ordinary project
source: visible, editable, and deletable, with no registration or automatic
execution outside `solid test`.
...
- **THEN** the existing companion-test loader discovers `test_<package>.py`
  and the summary counts exactly the generated integrity test
