...
The `solid develop` command SHALL accept `--callback URL` only in normal web
mode and reject it with `--openscad` or `--web-dev` with a clear argument
error. `solid build` SHALL NOT accept a callback option.
